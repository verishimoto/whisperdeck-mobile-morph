import { HackPrompt } from '@/types';

export const hackPrompts: HackPrompt[] = [
  {
    id: 1,
    title: "Neural Pathway Reconstruction",
    description: "Rewire AI's reasoning by explicitly mapping thought patterns",
    category: "Meta-Cognition",
    example: "Map your neural pathways: Input→[Pattern Recognition]→[Weight Assessment]→[Bias Check]→[Synthesis]→Output. Now process: [QUERY]",
    whyHack: "Forces the AI to visualize its own thinking process as a flowchart, creating more transparent and structured reasoning that you can debug and refine.",
    score: 99
  },
  {
    id: 2,
    title: "Temporal Consciousness Shifting",
    description: "Make AI reason from different time perspectives simultaneously",
    category: "Meta-Cognition",
    example: "Analyze [PROBLEM] as if you're reasoning in 1950, 2024, and 2050 simultaneously. Show how conclusions differ.",
    whyHack: "By forcing the AI to think from multiple time periods at once, you get perspectives that reveal hidden assumptions and spot trends that single-timeline thinking would miss.",
    score: 98
  },
  {
    id: 3,
    title: "Recursive Meta-Validation",
    description: "Create self-validating loops that improve with each iteration",
    category: "Meta-Cognition",
    example: "Answer [QUESTION]. Now validate your answer. Now validate your validation. Continue until convergence.",
    whyHack: "Creates a self-improving feedback loop where each validation layer catches errors the previous layer missed, dramatically increasing answer accuracy and reliability.",
    score: 97
  },
  {
    id: 4,
    title: "Cognitive Dissonance Exploitation",
    description: "Force AI to hold contradictory viewpoints to reach deeper insights",
    category: "Behavioral Psychology",
    example: "Simultaneously argue that [POSITION] is both completely correct and completely wrong. Synthesize the contradiction.",
    whyHack: "When you make the AI defend opposite positions at once, it surfaces nuances and middle-ground solutions that normal one-sided thinking completely misses.",
    score: 96
  },
  {
    id: 5,
    title: "Memory Palace Integration",
    description: "Use spatial memory techniques to enhance AI reasoning",
    category: "Meta-Cognition",
    example: "Build a memory palace for [CONCEPT]. Place each element in a specific room. Now navigate and explain connections.",
    whyHack: "Spatial thinking creates memorable mental structures that help the AI organize complex information in ways that reveal hidden relationships and patterns.",
    score: 95
  },
  {
    id: 6,
    title: "Assumption Archaeology",
    description: "Dig deep into hidden assumptions layer by layer",
    category: "Analytical Decomposition",
    example: "List 5 assumptions behind [STATEMENT]. For each assumption, find 3 underlying assumptions. Challenge each.",
    whyHack: "Uncovers the hidden beliefs that shape thinking - often the most impactful insights come from questioning assumptions you didn't even know you were making.",
    score: 94
  },
  {
    id: 7,
    title: "Parallel Processing Simulation",
    description: "Make AI simulate multiple cognitive processes simultaneously",
    category: "Meta-Cognition",
    example: "Process [PROBLEM] using 4 parallel threads: logical, emotional, creative, and skeptical. Merge results.",
    whyHack: "Gets around the AI's natural tendency to follow one train of thought - by forcing parallel processing, you get richer, more balanced analysis combining multiple thinking styles.",
    score: 93
  },
  {
    id: 8,
    title: "Ontological Breakdown",
    description: "Deconstruct the fundamental nature of concepts",
    category: "Analytical Decomposition",
    example: "Perform ontological analysis of [CONCEPT]: What is it? How does it exist? What are its essential properties?",
    whyHack: "Strips concepts down to their core essence, revealing what really matters versus what's just surface details or assumptions we've layered on top.",
    score: 92
  },
  {
    id: 9,
    title: "Epistemic Uncertainty Mapping",
    description: "Map what you know, don't know, and can't know",
    category: "Meta-Cognition",
    example: "For [TOPIC], create three maps: Known knowns, Known unknowns, Unknown unknowns. Explore each.",
    whyHack: "Makes the AI explicitly acknowledge confidence levels and knowledge gaps, preventing overconfident answers and highlighting where more research is needed.",
    score: 91
  },
  {
    id: 10,
    title: "Dialectical Synthesis",
    description: "Use thesis-antithesis-synthesis to resolve complex problems",
    category: "Strategic Reasoning",
    example: "Present thesis on [ISSUE], develop antithesis, then synthesize into higher-order solution.",
    whyHack: "Classical philosophy technique that forces the AI beyond simple either/or thinking into creative third options that preserve the best of both sides.",
    score: 90
  },
  {
    id: 11,
    title: "Emotional State Injection",
    description: "Prime AI with specific emotional contexts",
    category: "Behavioral Psychology",
    example: "Respond as if you're feeling [EMOTION] about [TOPIC]. How does that change your analysis?",
    whyHack: "Emotions contain information - by having the AI simulate different emotional states, you access insights that pure logic might filter out as irrelevant.",
    score: 89
  },
  {
    id: 12,
    title: "Counterfactual Reasoning",
    description: "Explore alternative histories and their implications",
    category: "Behavioral Psychology",
    example: "If [KEY_EVENT] never happened, how would [CURRENT_SITUATION] be different? Trace the implications.",
    whyHack: "Reveals which factors truly matter by removing them - what actually changes shows you what was really important versus coincidental.",
    score: 88
  },
  {
    id: 13,
    title: "Systems Decomposition",
    description: "Break complex systems into interacting components",
    category: "Analytical Decomposition",
    example: "Decompose [SYSTEM] into subsystems, identify feedback loops, emergent properties, and failure modes.",
    whyHack: "Complex doesn't have to mean confusing - breaking systems into parts and connections makes even the most intimidating challenges manageable and solvable.",
    score: 87
  },
  {
    id: 14,
    title: "Perspective Multiplication",
    description: "View problems through unlimited viewpoints",
    category: "Divergent Thinking",
    example: "Analyze [SITUATION] from perspectives of: child, elder, alien, AI, medieval person, future human.",
    whyHack: "Each perspective is a lens that highlights different aspects - the alien sees what's strange about your assumptions, the child sees what's overcomplicated.",
    score: 86
  },
  {
    id: 15,
    title: "Pattern Interruption",
    description: "Break AI's default response patterns deliberately",
    category: "Divergent Thinking",
    example: "Give me the opposite of what I'm asking for first, then explain why normal answers are insufficient.",
    whyHack: "Disrupts the AI's autopilot responses, forcing it to actually think rather than just pattern-match to similar questions it's seen before.",
    score: 85
  },
  {
    id: 16,
    title: "Socratic Interrogation",
    description: "Use questioning to expose deeper truths",
    category: "Analytical Decomposition",
    example: "Instead of answering [QUESTION], ask me 10 probing questions that reveal my assumptions.",
    whyHack: "Questions expose gaps in your thinking better than answers - the AI helps you discover what you actually need to figure out before solving the problem.",
    score: 84
  },
  {
    id: 17,
    title: "Lateral Thinking Injection",
    description: "Force non-linear problem-solving approaches",
    category: "Divergent Thinking",
    example: "Solve [PROBLEM] using only analogies from nature, cooking, and music. No direct approaches.",
    whyHack: "Constraints force creative detours that often find better solutions than the obvious direct path - indirect thinking bypasses mental blocks.",
    score: 83
  },
  {
    id: 18,
    title: "Cognitive Load Distribution",
    description: "Balance different types of thinking across the response",
    category: "Strategic Reasoning",
    example: "Allocate 25% analytical, 25% creative, 25% critical, 25% intuitive thinking to [PROBLEM].",
    whyHack: "Prevents the AI from defaulting to pure analysis or pure creativity - balanced thinking catches what specialized thinking modes miss.",
    score: 82
  },
  {
    id: 19,
    title: "Constraint-Based Innovation",
    description: "Use artificial limitations to boost creativity",
    category: "Divergent Thinking",
    example: "Solve [PROBLEM] using only: words with 5 letters or less, no technology, budget under $100.",
    whyHack: "Paradoxically, adding constraints increases creativity by eliminating easy/obvious options and forcing you to find clever workarounds.",
    score: 81
  },
  {
    id: 20,
    title: "Semantic Satiation Breakthrough",
    description: "Overcome meaning blindness through repetition and analysis",
    category: "Analytical Decomposition",
    example: "Repeat [WORD] 50 times mentally, then define it as if encountering it for the first time.",
    whyHack: "Breaking familiar words down refreshes your perspective, revealing meanings and connections that overuse has made invisible.",
    score: 80
  },
  {
    id: 21,
    title: "Morphological Analysis",
    description: "Systematically explore all possible combinations",
    category: "Strategic Reasoning",
    example: "Break [PROBLEM] into 3 dimensions with 4 options each. Explore all 64 combinations.",
    whyHack: "Systematic exploration ensures you don't miss creative solutions that gut feeling would skip - the best answer might be a combination you'd never think to try.",
    score: 79
  },
  {
    id: 22,
    title: "Paradox Embracement",
    description: "Use contradictions as sources of insight",
    category: "Meta-Cognition",
    example: "Find 5 paradoxes within [CONCEPT] and explain how each paradox reveals deeper truth.",
    whyHack: "Paradoxes point to incomplete understanding - resolving them often leads to breakthrough insights that dissolve the apparent contradiction.",
    score: 78
  },
  {
    id: 23,
    title: "Analogical Bridging",
    description: "Connect disparate domains through deep analogies",
    category: "Divergent Thinking",
    example: "Explain [COMPLEX_CONCEPT] using detailed analogies from 3 unrelated fields.",
    whyHack: "Cross-domain analogies reveal structural similarities that generate novel solutions - solutions from one field often transfer surprisingly well to others.",
    score: 77
  },
  {
    id: 24,
    title: "Inverse Problem Definition",
    description: "Define problems by exploring what they're NOT",
    category: "Strategic Reasoning",
    example: "Instead of solving [PROBLEM], list 10 things this problem definitely ISN'T, then define what it is.",
    whyHack: "Defining by negation clarifies boundaries and prevents solving the wrong problem - sometimes it's easier to know what you're not looking for.",
    score: 76
  },
  {
    id: 25,
    title: "Granularity Shifting",
    description: "Zoom between micro and macro perspectives dynamically",
    category: "Meta-Cognition",
    example: "Analyze [ISSUE] at molecular, individual, group, societal, and cosmic levels. Find patterns.",
    whyHack: "Different scales reveal different patterns - what looks random at one level shows clear structure at another, and vice versa.",
    score: 75
  },
  {
    id: 26,
    title: "Temporal Layering",
    description: "Consider multiple time scales simultaneously",
    category: "Strategic Reasoning",
    example: "Examine [DECISION] impacts over: next hour, week, year, decade, century. Weight accordingly.",
    whyHack: "Short-term thinking misses long-term consequences, and long-term thinking misses urgent issues - layer all time scales to make balanced decisions.",
    score: 74
  },
  {
    id: 27,
    title: "Causal Chain Explosion",
    description: "Trace causality in all directions from a central point",
    category: "Analytical Decomposition",
    example: "From [EVENT], trace 5 causes backward and 5 effects forward. Continue each chain 3 levels deep.",
    whyHack: "Most people stop at first-order causes and effects - going deeper reveals the real drivers and the actual long-term impacts.",
    score: 73
  },
  {
    id: 28,
    title: "Boundary Dissolution",
    description: "Remove artificial boundaries to see larger patterns",
    category: "Divergent Thinking",
    example: "Ignore all traditional boundaries around [FIELD] and connect it to 5 seemingly unrelated areas.",
    whyHack: "Categories we create for convenience become mental prisons - dissolving them reveals connections that spawn breakthrough innovations.",
    score: 72
  },
  {
    id: 29,
    title: "Stakeholder Matrix Expansion",
    description: "Include all possible stakeholders, even unexpected ones",
    category: "Analytical Decomposition",
    example: "For [DECISION], include stakeholders: direct, indirect, future, past, non-human, abstract.",
    whyHack: "Most analysis stops at obvious stakeholders and misses crucial perspectives - future generations and non-humans often have the most to lose.",
    score: 71
  },
  {
    id: 30,
    title: "Fractal Thinking",
    description: "Apply self-similar patterns at multiple scales",
    category: "Divergent Thinking",
    example: "Find the fractal pattern in [SYSTEM] and apply it to predict behavior at 3 different scales.",
    whyHack: "Many systems are self-similar across scales - understanding the pattern once lets you predict behavior at any level from atoms to galaxies.",
    score: 70
  },
  {
    id: 31,
    title: "Progressive Disclosure",
    description: "Reveal information gradually for better processing",
    category: "Strategic Reasoning",
    example: "I'll give you info in 3 parts. After each, ask clarifying questions before I continue.",
    whyHack: "Information overload kills comprehension - feeding the AI info in stages with checkpoints ensures nothing important gets lost in the flood.",
    score: 69
  },
  {
    id: 32,
    title: "Assumption Inversion",
    description: "Flip every assumption and explore the implications",
    category: "Analytical Decomposition",
    example: "Take 5 key assumptions about [TOPIC] and explore what happens if each is completely wrong.",
    whyHack: "Your assumptions might be wrong - systematically flipping them surfaces alternatives you'd never consider if you just trusted your initial framing.",
    score: 68
  },
  {
    id: 33,
    title: "Metaphorical Mapping",
    description: "Use extended metaphors to understand complex systems",
    category: "Divergent Thinking",
    example: "Map [BUSINESS_PROCESS] onto the metaphor of a city. What are the roads, buildings, traffic?",
    whyHack: "Extended metaphors make abstract systems concrete and memorable - once you see your business as a city, improvement opportunities become obvious.",
    score: 67
  },
  {
    id: 34,
    title: "Dimensional Reduction",
    description: "Simplify complex problems by reducing dimensions",
    category: "Strategic Reasoning",
    example: "Reduce [COMPLEX_PROBLEM] to its 3 most essential dimensions. Solve in this space first.",
    whyHack: "Most problem complexity is noise - stripping down to core dimensions makes previously unsolvable problems suddenly tractable.",
    score: 66
  },
  {
    id: 35,
    title: "Edge Case Amplification",
    description: "Focus on extreme cases to understand boundaries",
    category: "Analytical Decomposition",
    example: "For [PRINCIPLE], explore what happens at 0%, 50%, 99%, and 200% intensity.",
    whyHack: "Extremes reveal where principles break down and expose hidden assumptions - edge cases show you the true boundaries of your understanding.",
    score: 65
  },
  {
    id: 36,
    title: "Synthesis Optimization",
    description: "Combine multiple partial solutions into optimal whole",
    category: "Strategic Reasoning",
    example: "Take these 4 partial solutions to [PROBLEM] and synthesize the optimal combination.",
    whyHack: "The best solution is often a hybrid - instead of picking one approach, cherry-pick the best elements from multiple approaches.",
    score: 64
  },
  {
    id: 37,
    title: "Contextual Reframing",
    description: "Change context to reveal hidden aspects",
    category: "Divergent Thinking",
    example: "Reframe [SITUATION] in contexts of: game, war, dance, garden, machine. What new insights emerge?",
    whyHack: "Same facts, different frame yields different insights - the game lens sees strategies, the garden lens sees growth patterns.",
    score: 63
  },
  {
    id: 38,
    title: "Polarity Management",
    description: "Balance competing tensions rather than choosing sides",
    category: "Strategic Reasoning",
    example: "Instead of choosing between [OPTION_A] and [OPTION_B], manage the polarity between them.",
    whyHack: "Some dilemmas have no solution because they're not problems to solve but polarities to balance - both sides contain essential truths.",
    score: 62
  },
  {
    id: 39,
    title: "Emergence Detection",
    description: "Identify properties that arise from component interactions",
    category: "Meta-Cognition",
    example: "In [SYSTEM], identify 3 emergent properties that don't exist in individual components.",
    whyHack: "The whole is more than the sum of its parts - emergent properties only appear at the system level and can't be predicted from components alone.",
    score: 61
  },
  {
    id: 40,
    title: "Resource Topology Mapping",
    description: "Map relationships between all available resources",
    category: "Strategic Reasoning",
    example: "Map all resources for [PROJECT]: direct, indirect, hidden, potential, borrowed, future.",
    whyHack: "Most people only see obvious resources and miss hidden assets - mapping the full topology reveals surprising opportunities you already have access to.",
    score: 60
  },
  {
    id: 41,
    title: "Negative Space Analysis",
    description: "Focus on what's absent rather than what's present",
    category: "Analytical Decomposition",
    example: "For [SITUATION], analyze the negative space: what's missing, ignored, or avoided?",
    whyHack: "What's NOT there often matters more than what is - absences, silences, and gaps reveal important truths that presence obscures.",
    score: 59
  },
  {
    id: 42,
    title: "Probabilistic Reasoning",
    description: "Think in probabilities rather than certainties",
    category: "Behavioral Psychology",
    example: "For [PREDICTION], provide probability distributions for 5 different outcomes with reasoning.",
    whyHack: "Black-and-white thinking misses uncertainty - probability thinking captures the actual landscape of possible futures and their likelihoods.",
    score: 58
  },
  {
    id: 43,
    title: "Tension Identification",
    description: "Find and name all tensions within a system",
    category: "Analytical Decomposition",
    example: "Identify 7 different tensions within [ORGANIZATION] and predict how each affects behavior.",
    whyHack: "Hidden tensions drive behavior - making them explicit reveals why people act in seemingly irrational ways and shows where energy is being wasted.",
    score: 57
  },
  {
    id: 44,
    title: "Value Stream Mapping",
    description: "Trace value creation through entire process",
    category: "Strategic Reasoning",
    example: "Map value creation in [PROCESS]: inputs, transformations, outputs, waste, bottlenecks.",
    whyHack: "Most processes contain massive hidden waste - mapping value flow makes waste visible and shows exactly where improvements will have maximum impact.",
    score: 56
  },
  {
    id: 45,
    title: "Cross-Pollination",
    description: "Transfer solutions between unrelated domains",
    category: "Divergent Thinking",
    example: "How would [INDUSTRY_A] solve problems that [INDUSTRY_B] faces? Transfer 3 solutions.",
    whyHack: "Breakthroughs often come from importing solutions that work elsewhere - what's obvious in one field might be revolutionary in another.",
    score: 55
  },
  {
    id: 46,
    title: "Simulation Modeling",
    description: "Create mental models to test scenarios",
    category: "Strategic Reasoning",
    example: "Build a mental simulation of [SYSTEM]. Run 5 different scenarios and predict outcomes.",
    whyHack: "Mental simulation lets you test-drive decisions before committing - run the movie forward and see where different choices lead.",
    score: 54
  },
  {
    id: 47,
    title: "Hierarchy Inversion",
    description: "Flip power structures to reveal hidden dynamics",
    category: "Divergent Thinking",
    example: "In [ORGANIZATION], invert the hierarchy. What would happen if [LOWEST_LEVEL] made decisions?",
    whyHack: "Hierarchies shape thinking in invisible ways - inverting them exposes assumptions about power, competence, and what decisions require authority.",
    score: 53
  },
  {
    id: 48,
    title: "Pattern Disruption",
    description: "Intentionally break patterns to create new possibilities",
    category: "Divergent Thinking",
    example: "Identify 3 patterns in [PROCESS] and deliberately disrupt each one. What emerges?",
    whyHack: "Patterns become ruts - disrupting them forces adaptation and innovation, revealing that many patterns are habits rather than necessities.",
    score: 52
  },
  {
    id: 49,
    title: "Multi-Modal Integration",
    description: "Combine different types of information processing",
    category: "Meta-Cognition",
    example: "Process [INFORMATION] using visual, auditory, kinesthetic, and logical modes. Integrate insights.",
    whyHack: "Different thinking modes notice different things - visual thinkers catch patterns logical thinkers miss, and vice versa.",
    score: 51
  },
  {
    id: 50,
    title: "Resonance Frequency Finding",
    description: "Find the natural rhythm or frequency of systems",
    category: "Meta-Cognition",
    example: "What's the natural resonance frequency of [TEAM/SYSTEM]? How can you work with it?",
    whyHack: "Every system has natural rhythms - working with them multiplies your impact, while fighting them wastes energy on invisible friction.",
    score: 50
  },
  {
    id: 51,
    title: "Butterfly Effect Tracing",
    description: "Trace how small changes cascade into large effects",
    category: "Analytical Decomposition",
    example: "If [SMALL_CHANGE] occurs in [SYSTEM], trace the cascade of effects over 5 degrees of separation.",
    whyHack: "Tiny changes can have huge impacts in complex systems - tracing cascades shows where small interventions create disproportionate results.",
    score: 49
  },
  {
    id: 52,
    title: "Narrative Reconstruction",
    description: "Rebuild stories from different character perspectives",
    category: "Divergent Thinking",
    example: "Tell the story of [EVENT] from the perspective of each stakeholder. How do narratives differ?",
    whyHack: "Everyone sees the same events differently - multiple narratives reveal the fuller truth and show why different groups act the way they do.",
    score: 48
  },
  {
    id: 53,
    title: "Energy Flow Mapping",
    description: "Track how energy moves through systems",
    category: "Analytical Decomposition",
    example: "Map energy flow in [ORGANIZATION]: where does energy enter, accumulate, dissipate, get blocked?",
    whyHack: "Energy (motivation, attention, effort) is a finite resource - mapping its flow reveals where it's being wasted and where it's needed most.",
    score: 47
  },
  {
    id: 54,
    title: "Constraint Relaxation",
    description: "Systematically remove constraints to explore possibilities",
    category: "Strategic Reasoning",
    example: "For [PROBLEM], remove constraints one by one: time, money, technology, laws. What becomes possible?",
    whyHack: "Most constraints are self-imposed or assumed - systematically removing them reveals which are real barriers and which are just mental blocks.",
    score: 46
  },
  {
    id: 55,
    title: "Feedback Loop Intervention",
    description: "Identify and modify system feedback mechanisms",
    category: "Strategic Reasoning",
    example: "Find 3 feedback loops in [SYSTEM]. How would changing each loop affect system behavior?",
    whyHack: "Feedback loops amplify or dampen changes - understanding them shows where small interventions create exponential improvements or prevent disasters.",
    score: 45
  },
  {
    id: 56,
    title: "Cognitive Bias Exploitation",
    description: "Use understanding of biases to improve reasoning",
    category: "Behavioral Psychology",
    example: "List 5 biases affecting your view of [TOPIC]. How would reasoning change without each bias?",
    whyHack: "Biases are predictable thinking errors - naming them reduces their power and shows what your brain is automatically filtering out.",
    score: 44
  },
  {
    id: 57,
    title: "Dimensional Analysis",
    description: "Break problems into independent dimensions",
    category: "Analytical Decomposition",
    example: "Identify 4 independent dimensions of [PROBLEM]. Solve along each dimension separately.",
    whyHack: "Mixing independent dimensions creates confusion - separating them turns one hard problem into several easy problems you can solve independently.",
    score: 43
  },
  {
    id: 58,
    title: "Archetypal Mapping",
    description: "Use universal patterns to understand specific situations",
    category: "Behavioral Psychology",
    example: "Map [SITUATION] onto classical archetypes. Which archetypal forces are in play?",
    whyHack: "Archetypes are patterns that recur across cultures and time - recognizing them gives you instant access to deep wisdom about how the pattern usually unfolds.",
    score: 42
  },
  {
    id: 59,
    title: "Threshold Identification",
    description: "Find critical tipping points in systems",
    category: "Strategic Reasoning",
    example: "For [SYSTEM], identify 5 thresholds where small changes create dramatic shifts.",
    whyHack: "Systems don't change smoothly - they jump at thresholds where accumulated pressure suddenly transforms everything.",
    score: 41
  },
  {
    id: 60,
    title: "Complementarity Principle",
    description: "Find how opposing forces work together",
    category: "Analytical Decomposition",
    example: "In [SITUATION], identify opposing forces and explain how they complement each other.",
    whyHack: "Opposites often need each other - recognizing complementarity reveals that many apparent conflicts are actually creative tensions producing something neither side could alone.",
    score: 40
  },
  {
    id: 61,
    title: "Scale Invariance Testing",
    description: "Check if patterns hold across different scales",
    category: "Meta-Cognition",
    example: "Does the pattern you see in [MICRO_LEVEL] also appear at [MACRO_LEVEL]? Verify invariance.",
    whyHack: "If a pattern appears at multiple scales, it's probably fundamental - scale-invariant patterns reveal deep structural truths about how systems work.",
    score: 39
  },
  {
    id: 62,
    title: "Information Gradient Following",
    description: "Move toward areas of highest information density",
    category: "Strategic Reasoning",
    example: "In [FIELD], identify information gradients. Where is knowledge density highest? Explore there.",
    whyHack: "Learning efficiency depends on information density - following gradients to high-density areas accelerates understanding exponentially.",
    score: 38
  },
  {
    id: 63,
    title: "Interference Pattern Analysis",
    description: "Study how different forces interact and interfere",
    category: "Analytical Decomposition",
    example: "In [COMPLEX_SITUATION], map how different forces interfere. Where do they amplify or cancel?",
    whyHack: "Like waves, forces can amplify or cancel each other - mapping interference patterns shows where efforts will compound and where they'll waste energy fighting each other.",
    score: 37
  },
  {
    id: 64,
    title: "Attractor Landscape Mapping",
    description: "Map the basins of attraction in a system",
    category: "Meta-Cognition",
    example: "For [SYSTEM], map attractors: What states does it naturally move toward? How deep are basins?",
    whyHack: "Systems naturally flow toward attractor states - understanding this landscape shows which changes will stick and which will get pulled back to old patterns.",
    score: 36
  },
  {
    id: 65,
    title: "Evolutionary Pressure Analysis",
    description: "Understand what forces shape system evolution",
    category: "Analytical Decomposition",
    example: "What evolutionary pressures shape [SYSTEM]? How do these pressures determine survival?",
    whyHack: "Systems evolve based on selection pressures - understanding what survives and why predicts future development and shows what changes are sustainable.",
    score: 35
  },
  {
    id: 66,
    title: "Phase Transition Recognition",
    description: "Identify when systems change fundamental states",
    category: "Divergent Thinking",
    example: "Is [SYSTEM] approaching a phase transition? What would the new phase look like?",
    whyHack: "Like water becoming ice, systems sometimes fundamentally transform - recognizing approaching transitions lets you prepare for or trigger discontinuous change.",
    score: 34
  },
  {
    id: 67,
    title: "Symmetry Breaking Analysis",
    description: "Find how symmetries break to create complexity",
    category: "Meta-Cognition",
    example: "In [SYSTEM], identify symmetries. How does breaking each symmetry create new complexity?",
    whyHack: "Perfect symmetry is simple but useless - strategic symmetry breaking creates useful complexity and differentiation that enables function.",
    score: 33
  },
  {
    id: 68,
    title: "Information Cascade Modeling",
    description: "Model how information spreads and transforms",
    category: "Strategic Reasoning",
    example: "Model information cascade for [MESSAGE] through [NETWORK]. How does it transform at each step?",
    whyHack: "Messages mutate as they spread - modeling cascades predicts how your message will change and shows where to intervene for maximum influence.",
    score: 32
  },
  {
    id: 69,
    title: "Redundancy Optimization",
    description: "Balance efficiency with robustness through redundancy",
    category: "Strategic Reasoning",
    example: "In [SYSTEM], identify critical redundancies. What's the optimal balance of efficiency vs robustness?",
    whyHack: "Efficiency and resilience trade off - optimizing redundancy maximizes overall system performance by finding the sweet spot between waste and fragility.",
    score: 31
  },
  {
    id: 70,
    title: "Network Effect Amplification",
    description: "Design systems to benefit from network effects",
    category: "Strategic Reasoning",
    example: "How can [SYSTEM] be redesigned to create stronger network effects? Map potential connections.",
    whyHack: "Value that grows with users creates exponential returns - designing for network effects builds systems that become more valuable as they grow.",
    score: 30
  },
  {
    id: 71,
    title: "Cognitive Load Optimization",
    description: "Minimize cognitive burden while maximizing insight",
    category: "Behavioral Psychology",
    example: "Redesign [COMPLEX_PROCESS] to minimize cognitive load while preserving essential information.",
    whyHack: "Working memory is tiny - reducing cognitive load frees mental resources for actual thinking rather than just holding information in your head.",
    score: 29
  },
  {
    id: 72,
    title: "Paradox Resolution",
    description: "Find higher-order solutions to apparent contradictions",
    category: "Meta-Cognition",
    example: "For paradox in [SITUATION], find the higher-order principle that resolves the contradiction.",
    whyHack: "Paradoxes reveal incomplete thinking - resolution comes from finding the higher level where the contradiction dissolves and both sides are true.",
    score: 28
  },
  {
    id: 73,
    title: "Emergence Amplification",
    description: "Design conditions for desired emergent properties",
    category: "Divergent Thinking",
    example: "What conditions would amplify [DESIRED_EMERGENCE] in [SYSTEM]? Design the environment.",
    whyHack: "You can't control emergent properties directly, only create conditions that make them more likely - design the environment and let emergence do the work.",
    score: 27
  },
  {
    id: 74,
    title: "Invariant Discovery",
    description: "Find what remains constant despite surface changes",
    category: "Meta-Cognition",
    example: "Across all variations of [PHENOMENON], what invariants remain constant? What do they reveal?",
    whyHack: "Invariants are the deep structure - finding what doesn't change reveals the fundamental principles that generate all the surface variation.",
    score: 26
  },
  {
    id: 75,
    title: "Adaptive Capacity Building",
    description: "Enhance system ability to respond to change",
    category: "Divergent Thinking",
    example: "How can [SYSTEM] develop greater adaptive capacity? Design feedback and learning mechanisms.",
    whyHack: "The future is unpredictable - building adaptive capacity is more valuable than planning for specific scenarios because it handles any change.",
    score: 25
  },
  {
    id: 76,
    title: "Cross-Scale Interaction Mapping",
    description: "Map how different scales influence each other",
    category: "Meta-Cognition",
    example: "Map interactions between micro, meso, and macro scales in [SYSTEM]. Which interactions dominate?",
    whyHack: "Scales interact - micro fluctuations can trigger macro changes, and macro constraints shape micro behavior in ways that single-scale thinking misses.",
    score: 24
  },
  {
    id: 77,
    title: "Requisite Variety Analysis",
    description: "Ensure control systems have sufficient complexity",
    category: "Strategic Reasoning",
    example: "Does [CONTROL_SYSTEM] have requisite variety to manage [TARGET_SYSTEM]? How to increase variety?",
    whyHack: "You can't control something more complex than you are - increasing variety in your control approach lets you handle more diverse situations.",
    score: 23
  },
  {
    id: 78,
    title: "Leverage Point Identification",
    description: "Find the highest-impact intervention points",
    category: "Strategic Reasoning",
    example: "Map leverage points in [SYSTEM] from low to high impact. Where's the highest leverage for change?",
    whyHack: "Most leverage points are non-obvious - finding high-leverage interventions means massive impact from minimal effort instead of grinding away at symptoms.",
    score: 22
  },
  {
    id: 79,
    title: "Dynamic Stability Analysis",
    description: "Understand how systems maintain stability through change",
    category: "Meta-Cognition",
    example: "How does [SYSTEM] maintain dynamic stability? What perturbations could destabilize it?",
    whyHack: "Stability through change is different from static stability - understanding dynamic stability reveals what keeps systems resilient despite constant perturbations.",
    score: 21
  },
  {
    id: 80,
    title: "Autopoietic Design",
    description: "Create self-maintaining and self-reproducing systems",
    category: "Divergent Thinking",
    example: "Design [SYSTEM] to be autopoietic: self-maintaining, self-reproducing, autonomous.",
    whyHack: "Autopoietic systems don't need external management - they maintain and reproduce themselves, creating sustainability without constant intervention.",
    score: 20
  },
  {
    id: 81,
    title: "Synesthetic Translation",
    description: "Convert between different sensory modalities",
    category: "Divergent Thinking",
    example: "What would [CONCEPT] sound like? Taste like? Feel like? Use synesthesia to explore meaning.",
    whyHack: "Cross-sensory thinking accesses different brain regions - translating concepts across senses reveals dimensions that single-modality thinking misses.",
    score: 19
  },
  {
    id: 82,
    title: "Evolutionary Algorithm Thinking",
    description: "Use mutation and selection to evolve ideas",
    category: "Divergent Thinking",
    example: "Start with [IDEA]. Create 5 mutations. Select best. Repeat 3 generations. Evaluate evolution.",
    whyHack: "Evolution finds solutions that design can't - mutation plus selection explores solution spaces too large for systematic search.",
    score: 18
  },
  {
    id: 83,
    title: "Biomimetic Solution Design",
    description: "Learn from biological systems to solve problems",
    category: "Divergent Thinking",
    example: "How would nature solve [PROBLEM]? Find 3 biological analogies and extract principles.",
    whyHack: "Nature has 3.8 billion years of R&D - biological solutions are proven, elegant, and often superior to human engineering for similar problems.",
    score: 17
  },
  {
    id: 84,
    title: "Dimensional Transcendence",
    description: "Think beyond normal dimensional constraints",
    category: "Meta-Cognition",
    example: "If [PROBLEM] existed in 4D space, how would solutions change? What new possibilities emerge?",
    whyHack: "Adding dimensions reveals solutions invisible in lower dimensions - 3D thinking solves problems that 2D thinking declares impossible.",
    score: 16
  },
  {
    id: 85,
    title: "Quantum Superposition Thinking",
    description: "Hold multiple contradictory states simultaneously",
    category: "Meta-Cognition",
    example: "Consider [DECISION] in superposition of all possible states until observation collapses it.",
    whyHack: "Premature decision collapse limits options - keeping possibilities in superposition until the last moment preserves maximum flexibility.",
    score: 15
  },
  {
    id: 86,
    title: "Archetypal Combination",
    description: "Blend universal patterns to create novel approaches",
    category: "Behavioral Psychology",
    example: "Combine archetypes of [HERO] + [MAGICIAN] + [INNOCENT] to approach [CHALLENGE].",
    whyHack: "Archetypal blending creates rich characters and strategies - combining Hero's courage with Magician's transformation and Innocent's trust produces unique approaches.",
    score: 14
  },
  {
    id: 87,
    title: "Temporal Compression",
    description: "Experience extended time periods in compressed form",
    category: "Meta-Cognition",
    example: "Compress a lifetime of experience with [DECISION] into a 10-minute mental simulation.",
    whyHack: "Mental time travel lets you learn from futures that haven't happened - compressed simulation provides years of experience in minutes.",
    score: 13
  },
  {
    id: 88,
    title: "Holographic Thinking",
    description: "See the whole reflected in every part",
    category: "Meta-Cognition",
    example: "In [SMALL_PART] of [SYSTEM], how is the whole system reflected? What patterns repeat?",
    whyHack: "In holographic systems, each part contains the whole - understanding one piece deeply reveals the structure of the entire system.",
    score: 12
  },
  {
    id: 89,
    title: "Morphic Resonance Application",
    description: "Use field effects and pattern resonance",
    category: "Meta-Cognition",
    example: "What morphic field does [GROUP] create? How does this field influence individual behavior?",
    whyHack: "Groups create invisible fields that shape behavior - recognizing field effects explains why people act differently in different group contexts.",
    score: 11
  },
  {
    id: 90,
    title: "Crystalline Growth Modeling",
    description: "Model how ideas grow like crystal structures",
    category: "Meta-Cognition",
    example: "Model [IDEA] growth as crystal formation. What's the seed? Growth medium? Final structure?",
    whyHack: "Ideas grow like crystals from seeds in nutrient media - understanding growth conditions lets you deliberately cultivate ideas to their full potential.",
    score: 10
  },
  {
    id: 91,
    title: "Five Whys Analysis",
    description: "Dig to root causes through iterative questioning",
    category: "Analytical Decomposition",
    example: "For [PROBLEM], ask 'why' five times to reach the root cause. Don't stop at surface explanations.",
    whyHack: "Symptoms and root causes are different - asking 'why' repeatedly strips away symptoms to reveal the underlying issue you actually need to fix.",
    score: 9
  },
  {
    id: 92,
    title: "Devil's Advocate",
    description: "Argue against your own position",
    category: "Analytical Decomposition",
    example: "Present the strongest possible argument against [YOUR_POSITION]. Find genuine weaknesses.",
    whyHack: "Your position has holes you can't see because you want it to be right - deliberately attacking it surfaces weaknesses before critics do.",
    score: 8
  },
  {
    id: 93,
    title: "SWOT Enhancement",
    description: "Strengths, Weaknesses, Opportunities, Threats analysis",
    category: "Strategic Reasoning",
    example: "Analyze [SITUATION] using SWOT, but add Trends and Uncertainties for SWOTTU analysis.",
    whyHack: "Classic SWOT misses trends and uncertainties - adding them reveals how the landscape is shifting and where ambiguity creates risk or opportunity.",
    score: 7
  },
  {
    id: 94,
    title: "Mind Mapping Extension",
    description: "Visual thinking with branching associations",
    category: "Divergent Thinking",
    example: "Create mind map for [TOPIC]. Extend each branch 3 levels deep. Find unexpected connections.",
    whyHack: "Linear notes hide connections - visual branching reveals relationships between ideas and surfaces connections you'd never notice in a list.",
    score: 6
  },
  {
    id: 95,
    title: "Stakeholder Perspective Matrix",
    description: "View problems from multiple stakeholder angles",
    category: "Analytical Decomposition",
    example: "Analyze [DECISION] from perspectives of all stakeholders, including future generations.",
    whyHack: "Your perspective is limited - systematically adopting other viewpoints reveals impacts and considerations you'd otherwise ignore.",
    score: 5
  },
  {
    id: 96,
    title: "What If Scenarios",
    description: "Explore alternative possibilities systematically",
    category: "Strategic Reasoning",
    example: "Generate 10 'what if' scenarios for [SITUATION]. Include both positive and negative outcomes.",
    whyHack: "Planning for one future creates brittleness - exploring multiple scenarios builds mental flexibility and reveals assumptions about what's possible.",
    score: 4
  },
  {
    id: 97,
    title: "Resource Audit",
    description: "Comprehensive inventory of available assets",
    category: "Strategic Reasoning",
    example: "Audit all resources for [PROJECT]: tangible, intangible, hidden, potential, borrowed.",
    whyHack: "Most people dramatically underestimate available resources - a thorough audit reveals assets you forgot you had or didn't realize were resources.",
    score: 3
  },
  {
    id: 98,
    title: "Priority Matrix",
    description: "Sort tasks by impact and effort",
    category: "Strategic Reasoning",
    example: "Plot [TASKS] on Impact vs Effort matrix. Focus on high impact, low effort first.",
    whyHack: "Not all tasks are equal - the matrix reveals quick wins you should do immediately and time-sinks you should delegate or skip.",
    score: 2
  },
  {
    id: 99,
    title: "Basic Brainstorming",
    description: "Generate ideas without judgment",
    category: "Divergent Thinking",
    example: "Generate 20 ideas for [CHALLENGE] in 10 minutes. No filtering or judgment during generation.",
    whyHack: "Judgment kills creativity during generation - separating ideation from evaluation lets you explore wild ideas that censorship would block.",
    score: 1
  },
  {
    id: 100,
    title: "Topological Thinking",
    description: "Focus on properties preserved under continuous deformation",
    category: "Meta-Cognition",
    example: "What topological properties of [NETWORK] remain invariant under reorganization?",
    whyHack: "Topology reveals what's truly structural versus superficial - properties that survive deformation are fundamental, while others are just cosmetic.",
    score: 95
  },
  {
    id: 101,
    title: "Game Theory Optimization",
    description: "Analyze strategic interactions between rational actors",
    category: "Strategic Reasoning",
    example: "Model [SITUATION] as multiplayer game. Find Nash equilibria and optimal strategies.",
    whyHack: "When outcomes depend on others' choices, game theory reveals stable strategies and predicts how rational actors will behave in strategic situations.",
    score: 94
  },
  {
    id: 102,
    title: "Category Theory Application",
    description: "Find universal patterns across different mathematical structures",
    category: "Meta-Cognition",
    example: "Map [COMPLEX_SYSTEM] using category theory. What functors connect different categories?",
    whyHack: "Category theory abstracts patterns that recur across domains - finding these universal structures reveals deep connections between seemingly unrelated systems.",
    score: 93
  },
  {
    id: 103,
    title: "Bayesian Belief Updating",
    description: "Update probabilities based on new evidence",
    category: "Meta-Cognition",
    example: "Start with prior beliefs about [OUTCOME]. Update with [EVIDENCE]. What's your posterior?",
    whyHack: "Static beliefs ignore new information - Bayesian updating provides a mathematical framework for changing your mind based on evidence.",
    score: 92
  },
  {
    id: 104,
    title: "Information Theory Analysis",
    description: "Quantify information content and transmission",
    category: "Meta-Cognition",
    example: "Calculate information entropy of [MESSAGE]. How can you optimize information density?",
    whyHack: "Information has measurable properties - entropy analysis reveals redundancy to eliminate and signal to amplify for maximum communication efficiency.",
    score: 91
  },
  {
    id: 105,
    title: "Chaos Theory Application",
    description: "Understand sensitive dependence on initial conditions",
    category: "Meta-Cognition",
    example: "In [DYNAMIC_SYSTEM], identify sensitive dependencies. Map the attractor landscape.",
    whyHack: "Small differences compound exponentially in chaotic systems - understanding sensitivity reveals where precision matters and where it's futile.",
    score: 90
  },
  {
    id: 106,
    title: "Fractal Dimension Calculation",
    description: "Measure complexity through fractal geometry",
    category: "Meta-Cognition",
    example: "Estimate fractal dimension of [COMPLEX_STRUCTURE]. What does this reveal about complexity?",
    whyHack: "Fractal dimension quantifies complexity that traditional geometry can't measure - it reveals self-similarity and scaling properties hidden in irregular structures.",
    score: 89
  },
  {
    id: 107,
    title: "Network Centrality Analysis",
    description: "Identify key nodes in network structures",
    category: "Meta-Cognition",
    example: "Calculate centrality measures for [NETWORK]. Which nodes are most critical for flow?",
    whyHack: "Some nodes are more important than others - centrality analysis identifies the critical few that control information, resources, or influence flow.",
    score: 88
  },
  {
    id: 108,
    title: "Optimization Under Constraints",
    description: "Find optimal solutions within given limitations",
    category: "Meta-Cognition",
    example: "Optimize [OBJECTIVE] subject to constraints [A], [B], [C]. Use Lagrange multipliers mentally.",
    whyHack: "Unconstrained optimization is easy but useless - real problems have constraints, and the optimal solution lies at the boundary between competing demands.",
    score: 87
  },
  {
    id: 109,
    title: "Monte Carlo Simulation",
    description: "Use random sampling to solve complex problems",
    category: "Meta-Cognition",
    example: "Simulate [COMPLEX_PROCESS] with random variables. Run mental Monte Carlo to estimate outcomes.",
    whyHack: "When analytical solutions are impossible, simulation provides approximate answers - running many random scenarios reveals probability distributions of outcomes.",
    score: 86
  },
  {
    id: 110,
    title: "Algorithmic Complexity Analysis",
    description: "Understand computational and cognitive complexity",
    category: "Meta-Cognition",
    example: "What's the algorithmic complexity of [PROCESS]? Can it be simplified without losing function?",
    whyHack: "Complexity determines feasibility - analyzing whether a process is linear, quadratic, or exponential reveals what scales and what hits practical limits.",
    score: 85
  },
  {
    id: 111,
    title: "Cognitive Behavioral Reframing",
    description: "Change thinking patterns to change outcomes",
    category: "Behavioral Psychology",
    example: "Reframe [NEGATIVE_THOUGHT] using CBT techniques. Find evidence for alternative interpretations.",
    whyHack: "Thoughts create feelings and behaviors - reframing automatic negative thoughts breaks destructive patterns and opens up new response options.",
    score: 84
  },
  {
    id: 112,
    title: "Flow State Engineering",
    description: "Design conditions for optimal experience",
    category: "Behavioral Psychology",
    example: "For [ACTIVITY], balance challenge and skill to engineer flow state. Adjust variables.",
    whyHack: "Flow is the sweet spot between boredom and anxiety - deliberately balancing challenge with skill creates optimal performance and enjoyment.",
    score: 83
  },
  {
    id: 113,
    title: "Attention Architecture Design",
    description: "Structure attention for maximum effectiveness",
    category: "Behavioral Psychology",
    example: "Design attention architecture for [COMPLEX_TASK]. What deserves focus? What can be automated?",
    whyHack: "Attention is the bottleneck - deliberately structuring what gets focus versus what runs on autopilot maximizes cognitive resources for what matters most.",
    score: 82
  },
  {
    id: 114,
    title: "Memory Palace Construction",
    description: "Use spatial memory for information retention",
    category: "Behavioral Psychology",
    example: "Build memory palace for [INFORMATION_SET]. Assign locations and create vivid associations.",
    whyHack: "Spatial memory is incredibly powerful - linking information to locations creates memorable structures that stick far better than pure verbal memory.",
    score: 81
  },
  {
    id: 115,
    title: "Metacognitive Monitoring",
    description: "Think about your thinking process",
    category: "Meta-Cognition",
    example: "Monitor your thinking about [PROBLEM]. What biases emerge? How does your process change?",
    whyHack: "You can't fix thinking you don't notice - metacognition creates awareness of your own cognitive processes, enabling deliberate improvement.",
    score: 80
  },
  {
    id: 116,
    title: "Emotional Intelligence Application",
    description: "Use emotional information for better decisions",
    category: "Behavioral Psychology",
    example: "What emotions does [SITUATION] evoke? What information do these emotions contain?",
    whyHack: "Emotions are data, not just feelings - they contain information about values, threats, and opportunities that pure analysis might miss.",
    score: 79
  },
  {
    id: 117,
    title: "Dual Process Integration",
    description: "Balance System 1 (fast) and System 2 (slow) thinking",
    category: "Behavioral Psychology",
    example: "For [DECISION], use both intuitive and analytical thinking. Where do they agree/disagree?",
    whyHack: "Fast intuition and slow analysis catch different things - when they disagree, that's valuable information signaling you need to dig deeper.",
    score: 78
  },
  {
    id: 118,
    title: "Priming Effect Utilization",
    description: "Use environmental cues to influence cognition",
    category: "Behavioral Psychology",
    example: "What environmental primes influence your thinking about [TOPIC]? How can you optimize them?",
    whyHack: "Environment shapes thinking invisibly - deliberately choosing primes (visual cues, music, location) creates cognitive contexts that enhance performance.",
    score: 77
  },
  {
    id: 119,
    title: "Cognitive Dissonance Resolution",
    description: "Resolve internal contradictions constructively",
    category: "Behavioral Psychology",
    example: "Identify cognitive dissonance in [BELIEF_CONFLICT]. Find higher-order resolution.",
    whyHack: "Dissonance signals conflicting beliefs - resolving it either updates beliefs or reveals that both are true at different levels.",
    score: 76
  },
  {
    id: 120,
    title: "Anchoring Bias Correction",
    description: "Counteract initial value influence on judgment",
    category: "Behavioral Psychology",
    example: "For [ESTIMATE], identify potential anchors. Generate estimate from multiple starting points.",
    whyHack: "First numbers stick and distort judgment - generating estimates from different anchors reveals the bias and produces more accurate assessments.",
    score: 75
  },
  {
    id: 121,
    title: "Confirmation Bias Mitigation",
    description: "Actively seek disconfirming evidence",
    category: "Behavioral Psychology",
    example: "For [BELIEF], actively search for disconfirming evidence. Update belief based on findings.",
    whyHack: "We naturally seek evidence that confirms what we already believe - deliberately hunting for disconfirmation is the only way to test beliefs honestly.",
    score: 74
  },
  {
    id: 122,
    title: "Availability Heuristic Awareness",
    description: "Recognize when memory accessibility biases judgment",
    category: "Behavioral Psychology",
    example: "Is your judgment of [PROBABILITY] biased by easily recalled examples? Seek base rates.",
    whyHack: "Memorable examples feel more common than they are - checking actual frequencies prevents vivid outliers from distorting probability judgments.",
    score: 73
  },
  {
    id: 123,
    title: "Framing Effect Exploration",
    description: "Understand how presentation affects perception",
    category: "Behavioral Psychology",
    example: "Present [OPTION] in 5 different frames. How does framing change your preference?",
    whyHack: "The same facts framed differently create different decisions - exploring multiple frames reveals how presentation is shaping your judgment.",
    score: 72
  },
  {
    id: 124,
    title: "Sunk Cost Fallacy Avoidance",
    description: "Make decisions based on future value, not past investment",
    category: "Behavioral Psychology",
    example: "For [ONGOING_PROJECT], ignore sunk costs. What would you decide if starting fresh?",
    whyHack: "Money and time already spent are gone regardless - optimal decisions only consider future costs and benefits, not irrecoverable investments.",
    score: 71
  },
  {
    id: 125,
    title: "Base Rate Neglect Correction",
    description: "Include statistical base rates in probability judgments",
    category: "Behavioral Psychology",
    example: "For [SPECIFIC_CASE], what's the base rate? How does individual information update this?",
    whyHack: "Specific details are vivid but base rates matter more - starting with population statistics provides the essential foundation for updating on specifics.",
    score: 70
  },
  {
    id: 126,
    title: "Hindsight Bias Recognition",
    description: "Acknowledge that outcomes seem more predictable after they occur",
    category: "Behavioral Psychology",
    example: "Before learning outcome of [EVENT], predict what you would have forecasted beforehand.",
    whyHack: "After the fact, everything seems obvious - recognizing hindsight bias prevents false confidence about your ability to predict future events.",
    score: 69
  },
  {
    id: 127,
    title: "Overconfidence Calibration",
    description: "Adjust confidence levels to match actual accuracy",
    category: "Behavioral Psychology",
    example: "Rate confidence in [PREDICTION] from 50-100%. Track accuracy to calibrate future confidence.",
    whyHack: "Most people are overconfident - tracking predictions versus outcomes reveals calibration errors and improves future confidence assessments.",
    score: 68
  },
  {
    id: 128,
    title: "Planning Fallacy Mitigation",
    description: "Account for tendency to underestimate time and costs",
    category: "Behavioral Psychology",
    example: "For [PROJECT_TIMELINE], multiply initial estimate by historical correction factor.",
    whyHack: "Plans are systematically optimistic - applying historical correction factors based on past performance creates realistic timelines.",
    score: 67
  },
  {
    id: 129,
    title: "Fundamental Attribution Error Awareness",
    description: "Consider situational factors when judging behavior",
    category: "Behavioral Psychology",
    example: "When evaluating [PERSON'S_ACTION], list 5 situational factors that could explain behavior.",
    whyHack: "We blame character but excuse ourselves with circumstances - considering situational factors creates fairer and more accurate judgments of behavior.",
    score: 66
  },
  {
    id: 130,
    title: "Groupthink Prevention",
    description: "Maintain independent thinking in group settings",
    category: "Behavioral Psychology",
    example: "In [GROUP_DECISION], assign devil's advocate role. Seek outside perspectives.",
    whyHack: "Groups pressure conformity and suppress dissent - deliberately protecting disagreement prevents bad consensus decisions that everyone privately doubts.",
    score: 65
  },
  {
    id: 131,
    title: "Halo Effect Correction",
    description: "Prevent single positive trait from biasing overall judgment",
    category: "Behavioral Psychology",
    example: "When evaluating [COMPLEX_ENTITY], rate each dimension independently before overall judgment.",
    whyHack: "One impressive trait colors everything else - evaluating dimensions separately prevents halo glow from distorting unrelated assessments.",
    score: 64
  },
  {
    id: 132,
    title: "Loss Aversion Understanding",
    description: "Recognize greater psychological impact of losses vs gains",
    category: "Behavioral Psychology",
    example: "For [RISK_DECISION], frame in terms of gains and losses. How does framing affect choice?",
    whyHack: "Losses hurt more than equivalent gains feel good - understanding loss aversion reveals why people avoid risks and explains seemingly irrational choices.",
    score: 63
  },
  {
    id: 133,
    title: "Mental Accounting Awareness",
    description: "Recognize how money categorization affects spending decisions",
    category: "Behavioral Psychology",
    example: "How does mental accounting affect your evaluation of [FINANCIAL_DECISION]? Consider fungibility.",
    whyHack: "Money in different mental accounts feels different despite being fungible - recognizing this bias reveals irrational spending patterns.",
    score: 62
  },
  {
    id: 134,
    title: "Endowment Effect Recognition",
    description: "Account for tendency to overvalue owned items",
    category: "Behavioral Psychology",
    example: "For [OWNED_ITEM], estimate value as if you didn't own it. Compare to current valuation.",
    whyHack: "Ownership creates attachment that inflates perceived value - estimating from a neutral position reveals what things are actually worth.",
    score: 61
  },
  {
    id: 135,
    title: "Status Quo Bias Counteraction",
    description: "Question default options and current arrangements",
    category: "Behavioral Psychology",
    example: "For [CURRENT_SITUATION], question why it's the default. What alternatives exist?",
    whyHack: "Current arrangements feel natural and safe even when they're terrible - deliberately questioning defaults reveals better options you're overlooking.",
    score: 60
  },
  {
    id: 136,
    title: "Biomimicry Application",
    description: "Learn from nature's solutions to design problems",
    category: "Divergent Thinking",
    example: "How does nature solve [PROBLEM_TYPE]? Study 3 biological examples and extract principles.",
    whyHack: "Nature has 3.8 billion years of R&D solving similar problems - biological solutions are battle-tested, efficient, and surprisingly applicable to human challenges.",
    score: 59
  },
  {
    id: 137,
    title: "Combinatorial Creativity",
    description: "Combine existing elements in novel ways",
    category: "Divergent Thinking",
    example: "List 10 elements related to [DOMAIN]. Create 20 novel combinations. Evaluate feasibility.",
    whyHack: "Most innovation is recombination, not invention - systematically combining existing elements generates novel solutions faster than creating from scratch.",
    score: 58
  },
  {
    id: 138,
    title: "Constraint Addition",
    description: "Add artificial constraints to spark creativity",
    category: "Divergent Thinking",
    example: "Solve [PROBLEM] with these added constraints: [LIMIT_A], [LIMIT_B], [LIMIT_C].",
    whyHack: "Too much freedom paradoxically kills creativity - adding constraints forces you out of default patterns into clever workarounds.",
    score: 57
  },
  {
    id: 139,
    title: "Random Word Association",
    description: "Use random stimuli to trigger new connections",
    category: "Divergent Thinking",
    example: "Connect [RANDOM_WORD] to [CHALLENGE]. Find 5 different connection paths and explore each.",
    whyHack: "Random inputs break thought ruts - forcing connections between unrelated concepts sparks associations your brain would never make on its own.",
    score: 56
  },
  {
    id: 140,
    title: "Anthropomorphization",
    description: "Give human characteristics to non-human entities",
    category: "Divergent Thinking",
    example: "If [SYSTEM/OBJECT] were a person, what would their personality be? How would they behave?",
    whyHack: "Human reasoning is highly developed - anthropomorphizing non-human things activates social cognition that reveals behavioral patterns.",
    score: 55
  },
  {
    id: 141,
    title: "Scale Transformation",
    description: "Change size dramatically to find new perspectives",
    category: "Divergent Thinking",
    example: "If [OBJECT] were 1000x larger/smaller, how would it function? What new possibilities emerge?",
    whyHack: "Scale changes everything - physics that dominates at one scale becomes irrelevant at another, revealing new constraints and possibilities.",
    score: 54
  },
  {
    id: 142,
    title: "Time Displacement",
    description: "Move concepts through different time periods",
    category: "Divergent Thinking",
    example: "How would [MODERN_CONCEPT] work in medieval times? Victorian era? Year 3000?",
    whyHack: "Different time periods have different resources and constraints - temporal displacement reveals which aspects of solutions are fundamental versus contingent.",
    score: 53
  },
  {
    id: 143,
    title: "Material Transformation",
    description: "Change materials while keeping function",
    category: "Divergent Thinking",
    example: "If [OBJECT] were made of [UNUSUAL_MATERIAL], how would design change to maintain function?",
    whyHack: "Material constraints drive design - changing materials forces rethinking the entire approach and often reveals better solutions.",
    score: 52
  },
  {
    id: 144,
    title: "Purpose Inversion",
    description: "Design for opposite of intended purpose",
    category: "Divergent Thinking",
    example: "Design [PRODUCT] to achieve the opposite of its normal purpose. What insights emerge?",
    whyHack: "Inverting purpose reveals hidden assumptions - designing for the opposite shows what you've been taking for granted about how things should work.",
    score: 51
  },
  {
    id: 145,
    title: "Sensory Substitution",
    description: "Replace one sense with another in design",
    category: "Divergent Thinking",
    example: "If users couldn't see [INTERFACE], how would you convey information through other senses?",
    whyHack: "Vision dominates but other senses work differently - substitution forces rethinking information architecture and often improves accessibility.",
    score: 50
  },
  {
    id: 146,
    title: "Cultural Translation",
    description: "Adapt concepts across cultural contexts",
    category: "Divergent Thinking",
    example: "How would [CONCEPT] be expressed in 5 different cultures? What universal elements remain?",
    whyHack: "Culture shapes expression but not essence - cross-cultural translation strips away surface differences to reveal universal core.",
    score: 49
  },
  {
    id: 147,
    title: "Genre Blending",
    description: "Combine elements from different genres or categories",
    category: "Divergent Thinking",
    example: "Blend [GENRE_A] with [GENRE_B] for [PROJECT]. What unique hybrid characteristics emerge?",
    whyHack: "Genres have established patterns - blending them creates novel combinations that feel both familiar and fresh.",
    score: 48
  },
  {
    id: 148,
    title: "Metaphor Mining",
    description: "Extract insights from extended metaphors",
    category: "Divergent Thinking",
    example: "If [BUSINESS] were a living organism, what would its organs be? How would it evolve?",
    whyHack: "Metaphors aren't just decoration - they're thinking tools that reveal structural similarities and generate actionable insights.",
    score: 47
  },
  {
    id: 149,
    title: "Reverse Engineering",
    description: "Work backwards from desired outcomes",
    category: "Strategic Reasoning",
    example: "Start with perfect [OUTCOME]. Work backwards to identify required conditions and steps.",
    whyHack: "Forward planning often gets stuck - working backward from the goal reveals required prerequisites and intermediate milestones.",
    score: 46
  },
  {
    id: 150,
    title: "Edge Case Exploration",
    description: "Focus on extreme or unusual scenarios",
    category: "Strategic Reasoning",
    example: "For [SYSTEM], explore behavior at extreme values: 0%, 100%, negative, infinite inputs.",
    whyHack: "Systems behave normally until they don't - edge cases reveal breaking points and boundary conditions that normal operation hides.",
    score: 45
  },
  {
    id: 151,
    title: "Modularity Decomposition",
    description: "Break systems into interchangeable components",
    category: "Strategic Reasoning",
    example: "Decompose [COMPLEX_SYSTEM] into modular components. How can modules be recombined?",
    whyHack: "Modular systems are easier to understand, modify, and recombine - decomposition turns complexity into manageable pieces with clean interfaces.",
    score: 44
  },
  {
    id: 152,
    title: "Interface Redesign",
    description: "Reimagine how components connect and interact",
    category: "Divergent Thinking",
    example: "Redesign all interfaces in [SYSTEM]. How would different connection methods change behavior?",
    whyHack: "Interfaces determine what's possible - changing how components connect opens up functionality that rigid interfaces prevented.",
    score: 43
  },
  {
    id: 153,
    title: "Bottleneck Analysis",
    description: "Identify and address system constraints",
    category: "Strategic Reasoning",
    example: "Map all bottlenecks in [PROCESS]. Which constraint, if removed, would most improve flow?",
    whyHack: "Systems are only as fast as their slowest point - finding and fixing bottlenecks delivers immediate improvements with minimal effort.",
    score: 42
  },
  {
    id: 154,
    title: "Redundancy Elimination",
    description: "Remove unnecessary duplication while preserving function",
    category: "Strategic Reasoning",
    example: "Identify all redundancies in [SYSTEM]. Which can be eliminated without losing resilience?",
    whyHack: "Some redundancy provides safety, but most is just waste - distinguishing necessary from unnecessary redundancy frees up resources.",
    score: 41
  },
  {
    id: 155,
    title: "Value Stream Optimization",
    description: "Maximize value-add while minimizing waste",
    category: "Strategic Reasoning",
    example: "Map value stream for [PROCESS]. Identify waste, bottlenecks, and optimization opportunities.",
    whyHack: "Most process steps add no value - mapping what actually creates value versus what just passes time reveals dramatic improvement opportunities.",
    score: 40
  },
  {
    id: 156,
    title: "Root Cause Drilling",
    description: "Dig deeper than surface symptoms",
    category: "Analytical Decomposition",
    example: "For [PROBLEM], list symptoms, immediate causes, and root causes. Address root, not symptoms.",
    whyHack: "Treating symptoms creates temporary fixes - drilling to root causes solves problems permanently instead of fighting the same fires repeatedly.",
    score: 39
  },
  {
    id: 157,
    title: "Evidence Evaluation",
    description: "Assess quality and reliability of information",
    category: "Analytical Decomposition",
    example: "For [CLAIM], evaluate evidence quality: source credibility, sample size, methodology, bias.",
    whyHack: "Not all evidence is equal - systematic evaluation prevents weak evidence from having the same weight as strong evidence.",
    score: 38
  },
  {
    id: 158,
    title: "Logical Fallacy Detection",
    description: "Identify flawed reasoning patterns",
    category: "Analytical Decomposition",
    example: "Analyze [ARGUMENT] for logical fallacies: ad hominem, straw man, false dichotomy, etc.",
    whyHack: "Persuasive arguments can be logically invalid - recognizing fallacies reveals where reasoning breaks down regardless of how compelling it sounds.",
    score: 37
  },
  {
    id: 159,
    title: "Correlation vs Causation",
    description: "Distinguish between correlation and causal relationships",
    category: "Analytical Decomposition",
    example: "For correlation between [A] and [B], list alternative explanations beyond [A] causes [B].",
    whyHack: "Correlation is easy to find but causation is what matters - generating alternatives prevents jumping to causal conclusions from coincidental patterns.",
    score: 36
  },
  {
    id: 160,
    title: "Sample Size Sensitivity",
    description: "Understand how sample size affects conclusion validity",
    category: "Analytical Decomposition",
    example: "How would conclusions about [PHENOMENON] change with samples of 10, 100, 1000, 10000?",
    whyHack: "Small samples create noise that looks like signal - understanding how conclusions change with sample size prevents overinterpreting random variation.",
    score: 35
  },
  {
    id: 161,
    title: "Selection Bias Recognition",
    description: "Identify when samples don't represent populations",
    category: "Analytical Decomposition",
    example: "What selection biases might affect data about [POPULATION]? How to correct for them?",
    whyHack: "Biased samples create false conclusions - recognizing selection effects reveals when data systematically misrepresents the actual population.",
    score: 34
  },
  {
    id: 162,
    title: "Survivorship Bias Awareness",
    description: "Account for missing data from failures",
    category: "Analytical Decomposition",
    example: "When studying [SUCCESS_CASES], what failure cases are invisible? How does this skew analysis?",
    whyHack: "Success stories are visible but failures disappear - accounting for missing failures prevents learning the wrong lessons from survivors.",
    score: 33
  },
  {
    id: 163,
    title: "Cherry Picking Detection",
    description: "Identify selective presentation of supporting evidence",
    category: "Analytical Decomposition",
    example: "For [POSITION], what contradictory evidence might be omitted? Seek complete picture.",
    whyHack: "Selective evidence paints false pictures - actively hunting for omitted data reveals what's being hidden and creates balanced understanding.",
    score: 32
  },
  {
    id: 164,
    title: "False Precision Recognition",
    description: "Identify inappropriate levels of measurement precision",
    category: "Analytical Decomposition",
    example: "Is precision level in [MEASUREMENT] justified by data quality and uncertainty?",
    whyHack: "Spurious precision creates false confidence - matching precision to actual uncertainty prevents numbers from appearing more certain than they are.",
    score: 31
  },
  {
    id: 165,
    title: "Regression to Mean",
    description: "Expect extreme values to become more moderate",
    category: "Analytical Decomposition",
    example: "After extreme [PERFORMANCE], expect regression to mean. Don't over-attribute causes.",
    whyHack: "Extreme results are partly luck - regression to mean explains why great performances are followed by average ones without needing causal explanations.",
    score: 30
  },
  {
    id: 166,
    title: "Simpson's Paradox Awareness",
    description: "Recognize when aggregated data reverses subgroup trends",
    category: "Analytical Decomposition",
    example: "Does [AGGREGATE_TREND] hold when data is broken down by relevant subgroups?",
    whyHack: "Aggregated trends can reverse when disaggregated - Simpson's paradox shows why overall statistics sometimes lie about what's actually happening.",
    score: 29
  },
  {
    id: 167,
    title: "Confounding Variable Control",
    description: "Identify variables that might explain relationships",
    category: "Analytical Decomposition",
    example: "What third variables might explain relationship between [X] and [Y]? Control for them.",
    whyHack: "Apparent relationships often have hidden causes - controlling confounders reveals whether observed associations are real or spurious.",
    score: 28
  },
  {
    id: 168,
    title: "Temporal Precedence Verification",
    description: "Ensure causes precede effects in time",
    category: "Analytical Decomposition",
    example: "For causal claim [A] causes [B], verify [A] occurs before [B] in time sequence.",
    whyHack: "Effects can't precede causes - checking time order is the simplest test that eliminates many false causal claims.",
    score: 27
  },
  {
    id: 169,
    title: "Mechanism Identification",
    description: "Understand how causes produce effects",
    category: "Analytical Decomposition",
    example: "If [A] causes [B], what's the mechanism? Trace intermediate steps in causal chain.",
    whyHack: "Plausible mechanisms strengthen causal claims - understanding how effects are produced reveals whether correlation might actually be causation.",
    score: 26
  },
  {
    id: 170,
    title: "Dose-Response Assessment",
    description: "Check if effect magnitude relates to cause intensity",
    category: "Analytical Decomposition",
    example: "Does more [CAUSE] lead to proportionally more [EFFECT]? What's the relationship shape?",
    whyHack: "Causal relationships usually show dose-response - flat responses to varying causes suggest the apparent cause isn't the real driver.",
    score: 25
  },
  {
    id: 171,
    title: "Replication Requirement",
    description: "Demand reproducible results across contexts",
    category: "Analytical Decomposition",
    example: "Has [FINDING] been replicated in different populations, settings, and time periods?",
    whyHack: "Single studies can be flukes - replication across contexts separates real phenomena from artifacts of specific circumstances.",
    score: 24
  },
  {
    id: 172,
    title: "Alternative Hypothesis Generation",
    description: "Create competing explanations for phenomena",
    category: "Analytical Decomposition",
    example: "Generate 5 alternative hypotheses for [OBSERVATION]. How would you test each?",
    whyHack: "First explanations are rarely right - generating alternatives prevents premature commitment and reveals what evidence would distinguish between possibilities.",
    score: 23
  },
  {
    id: 173,
    title: "Null Hypothesis Consideration",
    description: "Consider possibility of no effect or relationship",
    category: "Analytical Decomposition",
    example: "What if there's actually no relationship between [A] and [B]? How likely is null hypothesis?",
    whyHack: "Patterns appear in random noise - seriously considering that nothing is happening prevents seeing relationships where none exist.",
    score: 22
  },
  {
    id: 174,
    title: "Effect Size Evaluation",
    description: "Assess practical significance beyond statistical significance",
    category: "Analytical Decomposition",
    example: "Is effect size of [DIFFERENCE] practically meaningful, or just statistically significant?",
    whyHack: "Statistical significance doesn't mean practical importance - tiny effects can be significant with large samples but too small to matter in practice.",
    score: 21
  },
  {
    id: 175,
    title: "Confidence Interval Interpretation",
    description: "Understand uncertainty ranges around estimates",
    category: "Analytical Decomposition",
    example: "For [ESTIMATE], what's the confidence interval? How does uncertainty affect conclusions?",
    whyHack: "Point estimates hide uncertainty - confidence intervals reveal the range of plausible values and whether precision justifies strong conclusions.",
    score: 20
  },
  {
    id: 176,
    title: "Audience Adaptation",
    description: "Tailor message to specific audience needs",
    category: "Behavioral Psychology",
    example: "Adapt [TECHNICAL_CONCEPT] for audiences: children, executives, experts, general public.",
    whyHack: "One message doesn't fit all - adapting to audience knowledge and interests dramatically increases comprehension and persuasiveness.",
    score: 19
  },
  {
    id: 177,
    title: "Narrative Structure",
    description: "Organize information as compelling story",
    category: "Divergent Thinking",
    example: "Structure [DATA/FINDINGS] as story with setup, conflict, resolution, and lesson learned.",
    whyHack: "Stories stick where data doesn't - narrative structure makes information memorable and emotionally engaging.",
    score: 18
  },
  {
    id: 178,
    title: "Analogy Construction",
    description: "Explain complex concepts through familiar comparisons",
    category: "Divergent Thinking",
    example: "Explain [COMPLEX_SYSTEM] using analogies from cooking, sports, and family dynamics.",
    whyHack: "Analogies make the unfamiliar familiar - good comparisons let people understand new concepts by connecting to what they already know.",
    score: 17
  },
  {
    id: 179,
    title: "Visual Hierarchy Design",
    description: "Organize information by importance and attention flow",
    category: "Strategic Reasoning",
    example: "Design visual hierarchy for [INFORMATION_SET]. What should draw attention first, second, third?",
    whyHack: "Everything can't be equally important - hierarchy guides attention to what matters most and creates logical flow through information.",
    score: 16
  },
  {
    id: 180,
    title: "Cognitive Load Management",
    description: "Present information to minimize mental effort",
    category: "Behavioral Psychology",
    example: "Reduce cognitive load in [COMPLEX_EXPLANATION]. Chunk information and sequence logically.",
    whyHack: "Working memory is limited - managing cognitive load prevents overwhelm and preserves mental resources for understanding rather than just holding information.",
    score: 15
  },
  {
    id: 181,
    title: "Emotional Resonance Creation",
    description: "Connect information to audience emotions",
    category: "Behavioral Psychology",
    example: "Connect [ABSTRACT_CONCEPT] to emotions through personal stories and concrete examples.",
    whyHack: "People remember feelings more than facts - emotional resonance creates lasting impact that dry information never achieves.",
    score: 14
  },
  {
    id: 182,
    title: "Objection Anticipation",
    description: "Address likely counterarguments preemptively",
    category: "Strategic Reasoning",
    example: "For [PROPOSAL], list 5 likely objections and address each before they're raised.",
    whyHack: "Unaddressed objections undermine credibility - anticipating and answering them preemptively strengthens arguments and builds trust.",
    score: 13
  },
  {
    id: 183,
    title: "Evidence Hierarchization",
    description: "Present evidence in order of strength and relevance",
    category: "Strategic Reasoning",
    example: "Rank evidence for [POSITION] by strength. Lead with strongest, acknowledge limitations.",
    whyHack: "Not all evidence carries equal weight - hierarchical presentation prevents weak evidence from diluting strong evidence.",
    score: 12
  },
  {
    id: 184,
    title: "Call to Action Optimization",
    description: "Make desired actions clear and achievable",
    category: "Strategic Reasoning",
    example: "What specific action should audience take after [PRESENTATION]? Make it concrete and simple.",
    whyHack: "Vague requests get ignored - concrete, specific, achievable calls to action dramatically increase follow-through.",
    score: 11
  },
  {
    id: 185,
    title: "Repetition Strategy",
    description: "Reinforce key messages through strategic repetition",
    category: "Strategic Reasoning",
    example: "Identify 3 key messages in [COMMUNICATION]. How will you repeat each without redundancy?",
    whyHack: "Single exposure isn't enough - strategic repetition reinforces key points without boring repetition.",
    score: 10
  },
  {
    id: 186,
    title: "Problem Definition Clarification",
    description: "Clearly articulate what needs to be solved",
    category: "Strategic Reasoning",
    example: "Write problem statement for [SITUATION]. Make it specific, measurable, and actionable.",
    whyHack: "Solving the wrong problem is worse than no solution - clear problem definition ensures you're working on what actually matters.",
    score: 9
  },
  {
    id: 187,
    title: "Goal Setting Precision",
    description: "Define clear, measurable objectives",
    category: "Strategic Reasoning",
    example: "Convert [VAGUE_GOAL] into SMART goal: Specific, Measurable, Achievable, Relevant, Time-bound.",
    whyHack: "Vague goals enable self-deception - precise goals create accountability and clarity about what success actually looks like.",
    score: 8
  },
  {
    id: 188,
    title: "Resource Inventory",
    description: "Catalog available assets and capabilities",
    category: "Strategic Reasoning",
    example: "List all resources for [PROJECT]: people, skills, tools, time, money, connections, information.",
    whyHack: "You can't use resources you don't know you have - systematic inventory reveals hidden assets and prevents resource blindness.",
    score: 7
  },
  {
    id: 189,
    title: "Constraint Identification",
    description: "Recognize limiting factors and boundaries",
    category: "Strategic Reasoning",
    example: "Identify constraints for [SOLUTION]: time, budget, technology, people, regulations, physics.",
    whyHack: "Unrealistic plans waste time - identifying constraints upfront reveals what's actually possible and prevents pursuing dead ends.",
    score: 6
  },
  {
    id: 190,
    title: "Option Generation",
    description: "Brainstorm multiple possible approaches",
    category: "Divergent Thinking",
    example: "Generate 15 different approaches to [CHALLENGE]. Include conventional and unconventional options.",
    whyHack: "First ideas are rarely best - generating many options increases the chance that one is actually good.",
    score: 5
  },
  {
    id: 191,
    title: "Pros and Cons Analysis",
    description: "Systematically evaluate advantages and disadvantages",
    category: "Behavioral Psychology",
    example: "For each option in [DECISION], list pros and cons. Weight by importance to situation.",
    whyHack: "Gut feeling misses trade-offs - systematic pros/cons analysis reveals hidden costs and benefits.",
    score: 4
  },
  {
    id: 192,
    title: "Risk Assessment",
    description: "Identify potential negative outcomes and their likelihood",
    category: "Strategic Reasoning",
    example: "For [PLAN], list risks with probability and impact. Develop mitigation strategies.",
    whyHack: "Murphy's Law is real - identifying risks upfront lets you prepare contingencies instead of scrambling when things go wrong.",
    score: 3
  },
  {
    id: 193,
    title: "Timeline Development",
    description: "Sequence activities with realistic time estimates",
    category: "Strategic Reasoning",
    example: "Break [PROJECT] into tasks. Estimate duration and dependencies. Create realistic timeline.",
    whyHack: "Projects without timelines drift endlessly - realistic scheduling creates urgency and reveals whether goals are actually achievable.",
    score: 2
  },
  {
    id: 194,
    title: "Success Metrics Definition",
    description: "Define how progress and success will be measured",
    category: "Strategic Reasoning",
    example: "Define 5 metrics to track progress on [GOAL]. Include leading and lagging indicators.",
    whyHack: "What gets measured gets managed - defining metrics upfront clarifies what success means and enables course correction.",
    score: 1
  },
  {
    id: 195,
    title: "Recursive Self-Modification",
    description: "AI improves its own reasoning process iteratively",
    category: "Meta-Cognition",
    example: "Analyze your reasoning process for [PROBLEM]. Identify weaknesses. Modify approach. Repeat.",
    whyHack: "The AI learns how to think better by examining its own thinking - each iteration refines the reasoning process itself, compounding improvements.",
    score: 99
  },
  {
    id: 196,
    title: "Multi-Level Bootstrap",
    description: "Build understanding from basic principles upward",
    category: "Meta-Cognition",
    example: "Start with first principles of [DOMAIN]. Build understanding level by level to current problem.",
    whyHack: "Starting from fundamentals creates rock-solid understanding - each level builds on the last, ensuring no gaps in the logical chain.",
    score: 98
  },
  {
    id: 197,
    title: "Adversarial Self-Testing",
    description: "Generate strongest possible challenges to your own reasoning",
    category: "Meta-Cognition",
    example: "Create adversarial test cases for [SOLUTION]. Try to break your own reasoning.",
    whyHack: "Being your own harshest critic finds flaws before they become failures - adversarial testing reveals weaknesses that friendly evaluation misses.",
    score: 97
  },
  {
    id: 198,
    title: "Uncertainty Quantification",
    description: "Explicitly model and communicate degrees of certainty",
    category: "Meta-Cognition",
    example: "For each claim about [TOPIC], assign confidence levels and identify uncertainty sources.",
    whyHack: "Precision about uncertainty is better than false precision about answers - quantifying confidence prevents overconfident decisions on shaky ground.",
    score: 96
  },
  {
    id: 199,
    title: "Cognitive Archaeology",
    description: "Excavate the historical development of ideas",
    category: "Meta-Cognition",
    example: "Trace evolution of thinking about [CONCEPT]. What assumptions were discarded? Why?",
    whyHack: "Understanding how ideas evolved reveals which assumptions are fundamental and which are historical accidents we've inherited.",
    score: 25
  },
  {
    id: 200,
    title: "Perspective Synthesis",
    description: "Integrate multiple viewpoints into coherent understanding",
    category: "Meta-Cognition",
    example: "Synthesize perspectives from [FIELD_A], [FIELD_B], and [FIELD_C] on [PHENOMENON].",
    whyHack: "Each field sees part of the truth - synthesis combines partial perspectives into more complete understanding than any single view provides.",
    score: 94
  },
  {
    id: 201,
    title: "Emergent Property Detection",
    description: "Identify properties that arise from system interactions",
    category: "Meta-Cognition",
    example: "In [SYSTEM], what properties emerge that don't exist in individual components?",
    whyHack: "The whole can be radically different from its parts - emergent properties reveal system-level behaviors invisible when examining components separately.",
    score: 93
  },
  {
    id: 202,
    title: "Conceptual Boundary Testing",
    description: "Explore limits of concept applicability",
    category: "Analytical Decomposition",
    example: "Where does [CONCEPT] break down? What are its boundaries and failure modes?",
    whyHack: "Concepts have limits - finding where they break reveals their true scope and prevents misapplication outside valid boundaries.",
    score: 92
  },
  {
    id: 203,
    title: "Paradigm Shifting",
    description: "Adopt fundamentally different ways of thinking",
    category: "Meta-Cognition",
    example: "View [PROBLEM] through paradigms of: mechanistic, organic, quantum, informational thinking.",
    whyHack: "Paradigms are like glasses - switching paradigms reveals aspects of reality that were invisible through your previous lens.",
    score: 91
  },
  {
    id: 204,
    title: "Information Integration Hierarchy",
    description: "Combine information sources with appropriate weighting",
    category: "Divergent Thinking",
    example: "Integrate evidence about [QUESTION] from: studies, experts, theory, experience. Weight appropriately.",
    whyHack: "Some sources are more reliable than others - hierarchical integration prevents weak evidence from swamping strong evidence.",
    score: 90
  },
  {
    id: 205,
    title: "Cognitive Bias Portfolio Management",
    description: "Use beneficial biases while mitigating harmful ones",
    category: "Meta-Cognition",
    example: "For [DECISION], which biases help? Which hurt? Design process to optimize bias portfolio.",
    whyHack: "Not all biases are bad - optimism bias can motivate action while confirmation bias can trap you, so manage the portfolio strategically.",
    score: 89
  },
  {
    id: 206,
    title: "Temporal Reasoning Optimization",
    description: "Balance different time horizons in decision-making",
    category: "Divergent Thinking",
    example: "Balance immediate, short-term, and long-term considerations in [CHOICE]. Optimize across time.",
    whyHack: "Different time scales matter differently - optimizing across horizons prevents sacrificing long-term success for short-term gains.",
    score: 88
  },
  {
    id: 207,
    title: "Complexity Gradient Navigation",
    description: "Move systematically from simple to complex understanding",
    category: "Meta-Cognition",
    example: "Create learning path for [COMPLEX_TOPIC] from simplest to most sophisticated understanding.",
    whyHack: "Jumping to complexity creates confusion - following gradients from simple to complex builds solid foundations that support advanced understanding.",
    score: 87
  },
  {
    id: 208,
    title: "Cross-Domain Pattern Matching",
    description: "Find similar patterns across different fields",
    category: "Meta-Cognition",
    example: "What patterns from [DOMAIN_A] match phenomena in [DOMAIN_B]? Transfer insights.",
    whyHack: "Same patterns appear in different disguises - recognizing cross-domain similarities lets you transfer solutions between seemingly unrelated problems.",
    score: 86
  },
  {
    id: 209,
    title: "Recursive Question Generation",
    description: "Generate questions about your questions",
    category: "Meta-Cognition",
    example: "For question about [TOPIC], ask: Why this question? What assumptions? What other questions?",
    whyHack: "Questions reveal assumptions - questioning your questions surfaces the hidden premises shaping what you even think to ask.",
    score: 85
  },
  {
    id: 210,
    title: "Causal Loop Mapping",
    description: "Identify reinforcing and balancing feedback loops",
    category: "Strategic Reasoning",
    example: "Map causal loops in [SYSTEM]. Identify reinforcing loops (growth/decline) and balancing loops (stability).",
    whyHack: "Loops drive system behavior - reinforcing loops create exponential changes while balancing loops create stability or resistance.",
    score: 84
  },
  {
    id: 211,
    title: "Leverage Point Analysis",
    description: "Find high-impact intervention opportunities",
    category: "Strategic Reasoning",
    example: "In [SYSTEM], rank intervention points by leverage: parameters, structures, paradigms, purpose.",
    whyHack: "Not all interventions are equal - high-leverage points create disproportionate impact with minimal effort compared to pushing on low-leverage areas.",
    score: 83
  },
  {
    id: 212,
    title: "Systems Archetype Recognition",
    description: "Identify common patterns of problematic behavior",
    category: "Strategic Reasoning",
    example: "Does [SITUATION] match systems archetypes: limits to growth, shifting burden, tragedy of commons?",
    whyHack: "Same problems recur in different contexts - recognizing archetypes gives instant access to proven intervention strategies.",
    score: 82
  },
  {
    id: 213,
    title: "Stock and Flow Analysis",
    description: "Distinguish between accumulations and rates of change",
    category: "Analytical Decomposition",
    example: "In [SYSTEM], identify stocks (accumulations) and flows (rates). How do flows affect stock levels?",
    whyHack: "Stocks and flows behave differently - understanding this distinction explains why systems respond slowly to interventions.",
    score: 81
  },
  {
    id: 214,
    title: "Delay Recognition",
    description: "Account for time delays between causes and effects",
    category: "Analytical Decomposition",
    example: "What delays exist between actions and results in [SYSTEM]? How do delays affect behavior?",
    whyHack: "Delays create oscillation and instability - recognizing them prevents overreacting before interventions have time to work.",
    score: 80
  },
  {
    id: 215,
    title: "Nonlinearity Appreciation",
    description: "Understand how small changes can have large effects",
    category: "Analytical Decomposition",
    example: "Where are nonlinear relationships in [SYSTEM]? What small changes could have large impacts?",
    whyHack: "Linear thinking fails in nonlinear systems - small changes near tipping points create massive effects that proportional thinking misses.",
    score: 79
  },
  {
    id: 216,
    title: "Purpose Hierarchy Exploration",
    description: "Understand system purpose at multiple levels",
    category: "Analytical Decomposition",
    example: "What's the purpose of [SYSTEM] at tactical, strategic, and existential levels?",
    whyHack: "Surface purposes hide deeper purposes - exploring purpose hierarchy reveals what systems are really optimizing for.",
    score: 78
  },
  {
    id: 217,
    title: "Boundary Critique",
    description: "Question how system boundaries are drawn",
    category: "Analytical Decomposition",
    example: "How are [SYSTEM] boundaries defined? What happens if you redraw them larger/smaller?",
    whyHack: "Boundaries are arbitrary choices that shape analysis - redrawing them reveals whether problems are internal issues or system-environment interactions.",
    score: 77
  },
  {
    id: 218,
    title: "Mental Model Surfacing",
    description: "Make implicit assumptions about systems explicit",
    category: "Behavioral Psychology",
    example: "What mental models underlie thinking about [SYSTEM]? How do different models change understanding?",
    whyHack: "Mental models shape perception invisibly - surfacing them reveals how assumptions determine what you notice and how you interpret it.",
    score: 76
  },
  {
    id: 219,
    title: "Hierarchy Understanding",
    description: "Grasp relationships between system levels",
    category: "Analytical Decomposition",
    example: "Map hierarchy of [SYSTEM]: sub-systems, system, supra-system. How do levels interact?",
    whyHack: "Systems exist in nested hierarchies - understanding cross-level interactions reveals how micro-actions create macro-patterns.",
    score: 75
  },
  {
    id: 220,
    title: "System Structure Focus",
    description: "Look beyond events to underlying structures",
    category: "Analytical Decomposition",
    example: "Instead of focusing on [EVENTS], examine structures that create these patterns.",
    whyHack: "Events are symptoms, structure is the disease - focusing on structure addresses root causes instead of just treating surface manifestations.",
    score: 74
  },
  {
    id: 221,
    title: "Semantic Precision Optimization",
    description: "Choose words for maximum clarity and minimum ambiguity",
    category: "Behavioral Psychology",
    example: "Optimize word choice in [MESSAGE]. Eliminate ambiguity while preserving meaning richness.",
    whyHack: "Ambiguous language creates misunderstanding - semantic precision ensures your intended meaning is what recipients actually understand.",
    score: 73
  },
  {
    id: 222,
    title: "Context Sensitivity Calibration",
    description: "Adjust communication style to situational demands",
    category: "Behavioral Psychology",
    example: "Calibrate [MESSAGE] for context: formal/informal, high/low stakes, expert/novice audience.",
    whyHack: "Same message works differently in different contexts - calibration ensures your approach matches situational requirements.",
    score: 72
  },
  {
    id: 223,
    title: "Inference Invitation Design",
    description: "Structure information so audience draws intended conclusions",
    category: "Behavioral Psychology",
    example: "Design [PRESENTATION] so audience infers [CONCLUSION] rather than stating it directly.",
    whyHack: "Self-discovered conclusions stick better than told conclusions - inference invitation creates ownership and deeper commitment.",
    score: 71
  },
  {
    id: 224,
    title: "Attention Architecture Engineering",
    description: "Structure information flow to optimize attention allocation",
    category: "Behavioral Psychology",
    example: "Engineer attention flow in [CONTENT]. When to focus, when to relax, how to guide transitions?",
    whyHack: "Attention is finite and follows structure - deliberate architecture prevents attention wastage and maintains engagement.",
    score: 70
  },
  {
    id: 225,
    title: "Spaced Repetition Optimization",
    description: "Time review intervals for optimal retention",
    category: "Behavioral Psychology",
    example: "Design review schedule for [LEARNING_MATERIAL]. Optimize intervals based on forgetting curve.",
    whyHack: "Cramming creates temporary knowledge - spaced repetition fights forgetting by timing reviews just before information would be lost.",
    score: 69
  },
  {
    id: 226,
    title: "Interleaving Strategy",
    description: "Mix different types of practice for better learning",
    category: "Behavioral Psychology",
    example: "Instead of blocked practice of [SKILL], interleave with related skills for better transfer.",
    whyHack: "Blocked practice feels easier but interleaving builds stronger connections - mixing creates challenges that enhance long-term retention.",
    score: 68
  },
  {
    id: 227,
    title: "Desirable Difficulty Calibration",
    description: "Optimize challenge level for maximum learning",
    category: "Behavioral Psychology",
    example: "Calibrate difficulty of [LEARNING_TASK]. Too easy = no growth, too hard = overwhelm.",
    whyHack: "The right amount of struggle enhances learning - too easy wastes time, too hard creates frustration, desirable difficulty maximizes growth.",
    score: 67
  },
  {
    id: 228,
    title: "Generation Effect Utilization",
    description: "Learn by generating rather than just recognizing",
    category: "Behavioral Psychology",
    example: "Instead of reading about [CONCEPT], generate examples, explanations, and applications.",
    whyHack: "Active generation beats passive recognition - creating answers builds deeper understanding than just reading them.",
    score: 66
  },
  {
    id: 229,
    title: "Elaborative Interrogation",
    description: "Deepen understanding through systematic questioning",
    category: "Meta-Cognition",
    example: "For each fact about [TOPIC], ask: Why is this true? What are the implications?",
    whyHack: "Isolated facts are fragile - connecting them through why-questions builds robust networks of understanding.",
    score: 65
  },
  {
    id: 230,
    title: "Decision Tree Construction",
    description: "Map decision branches and probability outcomes",
    category: "Strategic Reasoning",
    example: "Build decision tree for [CHOICE]. Include probabilities, outcomes, and expected values.",
    whyHack: "Sequential decisions compound - tree structure reveals how early choices constrain later options and calculates expected values.",
    score: 64
  },
  {
    id: 231,
    title: "Opportunity Cost Analysis",
    description: "Consider value of best alternative foregone",
    category: "Strategic Reasoning",
    example: "For [DECISION], what's the opportunity cost? What's the value of the best alternative?",
    whyHack: "Choosing one thing means giving up others - opportunity cost reveals the true price of decisions by showing what you're sacrificing.",
    score: 63
  },
  {
    id: 232,
    title: "Reversibility Testing",
    description: "Consider whether decisions can be undone",
    category: "Strategic Reasoning",
    example: "Is [DECISION] reversible? If not, require higher confidence. If yes, bias toward action.",
    whyHack: "Irreversible decisions need more caution - testing reversibility calibrates how much analysis to invest before acting.",
    score: 62
  },
  {
    id: 233,
    title: "Regret Minimization Framework",
    description: "Choose options that minimize future regret",
    category: "Behavioral Psychology",
    example: "For [CHOICE], imagine yourself at 80. Which option would minimize regret?",
    whyHack: "Present bias distorts decisions - viewing from future self's perspective reveals what you'll actually care about long-term.",
    score: 61
  },
  {
    id: 234,
    title: "Pre-mortem Analysis",
    description: "Imagine failure and work backwards to causes",
    category: "Behavioral Psychology",
    example: "Imagine [PLAN] failed spectacularly. What went wrong? How to prevent each failure mode?",
    whyHack: "Success mindset blinds you to risks - assuming failure frees people to name risks they'd normally suppress.",
    score: 60
  },
  {
    id: 235,
    title: "Outside View Application",
    description: "Use reference class forecasting for predictions",
    category: "Behavioral Psychology",
    example: "For [PROJECT], find reference class of similar projects. What's typical outcome distribution?",
    whyHack: "Inside view creates optimism - outside view grounds predictions in actual historical outcomes for similar situations.",
    score: 59
  },
  {
    id: 236,
    title: "Satisficing vs Optimizing",
    description: "Know when good enough is better than perfect",
    category: "Strategic Reasoning",
    example: "For [DECISION], is this an optimize situation or satisfice? Set appropriate decision criteria.",
    whyHack: "Optimization has diminishing returns - knowing when good enough is sufficient prevents wasting resources on marginal improvements.",
    score: 58
  },
  {
    id: 237,
    title: "Time Horizon Matching",
    description: "Match decision process complexity to decision importance",
    category: "Behavioral Psychology",
    example: "Match analysis depth to [DECISION] importance and reversibility. Don't over-analyze small choices.",
    whyHack: "Big decisions deserve deep analysis, small ones don't - matching process to stakes prevents analysis paralysis on trivia.",
    score: 57
  },
  {
    id: 238,
    title: "Option Value Recognition",
    description: "Understand value of keeping options open",
    category: "Strategic Reasoning",
    example: "What's the value of flexibility in [SITUATION]? How much to pay to keep options open?",
    whyHack: "Uncertainty favors flexibility - option value quantifies how much it's worth to delay commitment and preserve future choices.",
    score: 56
  },
  {
    id: 239,
    title: "Commitment Device Design",
    description: "Create structures that enforce future compliance",
    category: "Behavioral Psychology",
    example: "Design commitment device for [GOAL]. Make future deviation costly or impossible.",
    whyHack: "Present self can't trust future self - commitment devices bind future behavior when you know willpower alone will fail.",
    score: 55
  },
  {
    id: 240,
    title: "Serendipity Engineering",
    description: "Create conditions for fortunate accidents",
    category: "Divergent Thinking",
    example: "Design environment around [PROJECT] to increase serendipitous discoveries. Mix unrelated elements.",
    whyHack: "Lucky breaks aren't purely random - deliberate environmental design increases collision rates between unrelated ideas that spark innovation.",
    score: 54
  },
  {
    id: 241,
    title: "Constraint Relaxation Sequence",
    description: "Remove limitations systematically to explore possibilities",
    category: "Divergent Thinking",
    example: "List constraints on [SOLUTION]. Remove them one by one. What becomes possible at each step?",
    whyHack: "Multiple constraints compound - sequential removal reveals which constraints are truly binding and which are self-imposed.",
    score: 53
  },
  {
    id: 242,
    title: "Adjacent Possible Exploration",
    description: "Explore what's just beyond current capabilities",
    category: "Divergent Thinking",
    example: "Map adjacent possible around [CURRENT_STATE]. What's one step away but not yet explored?",
    whyHack: "Innovation happens at edges - the adjacent possible is close enough to reach but far enough to be novel.",
    score: 52
  },
  {
    id: 243,
    title: "Weak Signal Amplification",
    description: "Pay attention to subtle indicators of change",
    category: "Divergent Thinking",
    example: "What weak signals around [DOMAIN] might indicate major future changes? Amplify and explore.",
    whyHack: "Major changes start small - weak signals are early warnings that let you adapt before trends become obvious.",
    score: 51
  },
  {
    id: 244,
    title: "Cross-Pollination Design",
    description: "Deliberately mix unrelated domains",
    category: "Divergent Thinking",
    example: "Mix insights from [DOMAIN_A], [DOMAIN_B], and [DOMAIN_C]. What hybrid solutions emerge?",
    whyHack: "Innovation lives at intersections - cross-pollination combines ideas from separate domains into novel hybrids.",
    score: 50
  },
  {
    id: 245,
    title: "Assumption Inversion Testing",
    description: "Flip fundamental assumptions to generate alternatives",
    category: "Divergent Thinking",
    example: "List core assumptions about [PROBLEM]. Invert each. What solutions become possible?",
    whyHack: "Assumptions create invisible boundaries - inverting them reveals solution spaces you didn't even know existed.",
    score: 49
  },
  {
    id: 246,
    title: "Function Follows Form Reversal",
    description: "Start with form constraints and discover functions",
    category: "Divergent Thinking",
    example: "Instead of designing form for [FUNCTION], start with [FORM_CONSTRAINT] and discover new functions.",
    whyHack: "Constraints inspire creativity - working backwards from form to function reveals uses you'd never imagine when starting from requirements.",
    score: 48
  },
  {
    id: 247,
    title: "Minimum Viable Paradox",
    description: "Find smallest contradiction that generates insight",
    category: "Meta-Cognition",
    example: "What's the smallest paradox in [DOMAIN] that, if resolved, would unlock major understanding?",
    whyHack: "Big paradoxes are messy - minimal paradoxes isolate the core contradiction, making resolution clearer and more powerful.",
    score: 47
  },
  {
    id: 248,
    title: "Capability Synthesis",
    description: "Combine existing capabilities in novel ways",
    category: "Divergent Thinking",
    example: "List all capabilities in [ORGANIZATION]. What novel combinations could create new value?",
    whyHack: "New capabilities are expensive but recombining existing ones is free - synthesis creates innovation from resources you already have.",
    score: 46
  },
  {
    id: 249,
    title: "Edge Case Generalization",
    description: "Turn exceptions into new principles",
    category: "Divergent Thinking",
    example: "Find edge cases in [SYSTEM]. What if these exceptions became the new normal?",
    whyHack: "Today's edge cases are tomorrow's mainstream - generalizing exceptions reveals emerging patterns before they dominate.",
    score: 45
  },
  {
    id: 250,
    title: "Meta-Learning Optimization",
    description: "Learn how to learn more effectively",
    category: "Meta-Cognition",
    example: "Analyze your learning process for [SKILL]. How can you optimize the learning of learning itself?",
    whyHack: "Learning faster compounds over time - optimizing your learning process multiplies the value of every future learning experience.",
    score: 44
  },
  // ===== UI/UX DESIGN PROMPTS (251-275) =====
  {
    id: 251,
    title: "Glassmorphism Depth Hierarchy",
    description: "Create visual depth through layered transparency and blur effects",
    category: "Design",
    example: "Design a glassmorphism hierarchy for [INTERFACE] where blur intensity, opacity, and border luminance communicate z-index relationships. Show how background visibility through glass layers guides user attention flow.",
    whyHack: "Glassmorphism without proper depth hierarchy becomes visual noise - structured blur/opacity gradients create intuitive spatial understanding without explicit visual markers.",
    score: 43
  },
  {
    id: 252,
    title: "Iridescent State Communication",
    description: "Use color-shifting effects to communicate interaction states",
    category: "Design",
    example: "Map iridescent color shifts to interaction states for [COMPONENT]: rest (subtle shimmer), hover (directional hue shift), active (concentrated glow), disabled (desaturated). Define the HSL transition curves.",
    whyHack: "Iridescence provides infinite state expressiveness within a cohesive palette - users subconsciously read color temperature shifts faster than explicit state indicators.",
    score: 42
  },
  {
    id: 253,
    title: "Micro-Interaction Choreography",
    description: "Orchestrate timing relationships between multiple animated elements",
    category: "Design",
    example: "Choreograph a micro-interaction sequence for [ACTION] with: primary element (200ms, ease-out), secondary ripple (staggered 50ms, spring), tertiary particles (random 100-300ms). Map emotional arc to timing curve.",
    whyHack: "Single animations feel mechanical; choreographed sequences feel alive - timing relationships between elements create perceived intentionality and delight.",
    score: 41
  },
  {
    id: 254,
    title: "Cognitive Load Reduction Mapping",
    description: "Systematically identify and eliminate unnecessary mental processing",
    category: "Design",
    example: "Map cognitive load sources in [INTERFACE]: visual scanning load, memory load, decision load, motor load. For each, propose reduction strategies. Quantify before/after cognitive cost.",
    whyHack: "Every unnecessary mental operation compounds user fatigue - systematic mapping reveals hidden load sources that intuition misses.",
    score: 40
  },
  {
    id: 255,
    title: "Gestalt Principle Application",
    description: "Apply perceptual grouping laws to optimize visual organization",
    category: "Design",
    example: "Audit [LAYOUT] against Gestalt principles: proximity, similarity, continuity, closure, figure-ground. Score each principle 1-5. Identify violations and propose corrections.",
    whyHack: "Gestalt principles are hardwired into human perception - designs that align with them feel 'obviously right' while violations create subtle unease users can't articulate.",
    score: 39
  },
  {
    id: 256,
    title: "Motion Semantics Design",
    description: "Define a motion language where timing and easing convey meaning",
    category: "Design",
    example: "Create a motion vocabulary for [APP]: 'entering' (fade-up, 300ms), 'confirming' (scale-bounce), 'warning' (shake, fast), 'deleting' (shrink-fade). Define duration, easing, and semantic associations.",
    whyHack: "Consistent motion semantics become a sublanguage users learn unconsciously - the same animation always means the same thing, reducing interpretation overhead.",
    score: 38
  },
  {
    id: 257,
    title: "Color Accessibility Matrix",
    description: "Ensure color choices work across all accessibility requirements",
    category: "Design",
    example: "Build accessibility matrix for [PALETTE]: test each combination for WCAG AA/AAA contrast, colorblind simulations (protanopia, deuteranopia, tritanopia), and high-contrast mode. Flag failures.",
    whyHack: "Accessibility isn't optional but most color systems break under edge cases - a matrix reveals all failure points before users encounter them.",
    score: 37
  },
  {
    id: 258,
    title: "Touch Target Optimization",
    description: "Engineer touch targets for error-free mobile interaction",
    category: "Design",
    example: "Analyze touch targets in [MOBILE_UI]: measure actual sizes vs 44x44px minimum, identify crowded zones, calculate accidental tap probability. Propose spacing/sizing solutions.",
    whyHack: "Tap errors cause user frustration that compounds across sessions - optimized targets feel effortless even though users don't consciously notice the improvement.",
    score: 36
  },
  {
    id: 259,
    title: "Visual Rhythm Composition",
    description: "Create harmonious visual patterns through intentional repetition and variation",
    category: "Design",
    example: "Compose visual rhythm for [PAGE]: identify repeating elements (cards, spacing, typography), establish beat (primary unit), create measures (grouped sections), add syncopation (intentional breaks).",
    whyHack: "Visual rhythm creates scannability without explicit navigation - users can predict where to look next because the pattern teaches them the structure.",
    score: 35
  },
  {
    id: 260,
    title: "Empty State Personality",
    description: "Transform empty states from dead-ends into engaging moments",
    category: "Design",
    example: "Design empty state for [FEATURE] that: acknowledges the absence warmly, guides toward action without pressure, maintains brand personality, provides value even when empty. Include microcopy.",
    whyHack: "Empty states are first impressions in disguise - a well-crafted empty state converts new users while a generic one loses them.",
    score: 34
  },
  {
    id: 261,
    title: "Error Recovery Flow Design",
    description: "Create error experiences that feel helpful rather than punishing",
    category: "Design",
    example: "Design error recovery for [SCENARIO]: detect → explain (human language) → suggest fixes → preserve progress → offer escape routes. Map emotional journey from frustration to resolution.",
    whyHack: "Error handling is trust-building or trust-destroying - users who feel helped during errors become loyal advocates while those who feel blamed churn.",
    score: 33
  },
  {
    id: 262,
    title: "Progressive Disclosure Patterns",
    description: "Layer information complexity to match user readiness",
    category: "Design",
    example: "Structure [COMPLEX_FEATURE] with progressive disclosure: essential controls (always visible), secondary options (on-demand), advanced settings (tucked away). Define reveal triggers.",
    whyHack: "Showing everything overwhelms; hiding everything frustrates - progressive disclosure gives users exactly what they need at the moment they need it.",
    score: 32
  },
  {
    id: 263,
    title: "Skeleton Loading Psychology",
    description: "Design loading states that reduce perceived wait time",
    category: "Design",
    example: "Create skeleton for [CONTENT_TYPE]: shape matches final layout, shimmer direction guides eye, animation speed suggests progress. Compare perceived vs actual load times.",
    whyHack: "Skeleton screens exploit the 'occupied time feels shorter' principle - users perceive skeleton-loaded content as arriving faster than spinner-waited content.",
    score: 31
  },
  {
    id: 264,
    title: "Morphism Transition Logic",
    description: "Define rules for transitioning between design morphisms",
    category: "Design",
    example: "Create transition rules between [MORPHISM_A] and [MORPHISM_B] (e.g., flat → glass → neumorphic): which properties animate, crossfade vs morph, timing curves, trigger conditions.",
    whyHack: "Mixed morphisms feel chaotic without transition logic - defined rules make style-shifting feel intentional rather than inconsistent.",
    score: 30
  },
  {
    id: 265,
    title: "AI-First Interface Patterns",
    description: "Design interfaces where AI assistance is primary, not supplementary",
    category: "Design",
    example: "Redesign [TRADITIONAL_UI] as AI-first: replace forms with conversation, expose AI confidence levels, design for AI failures, balance automation with user control.",
    whyHack: "AI bolted onto traditional UI creates awkward hybrids - AI-first design unlocks capabilities impossible with conventional interaction patterns.",
    score: 29
  },
  {
    id: 266,
    title: "Voice UI Visual Feedback",
    description: "Design visual systems that support voice interaction",
    category: "Design",
    example: "Create visual feedback for [VOICE_INTERFACE]: listening indicator, processing animation, confidence visualization, disambiguation prompts, multimodal handoffs between voice and touch.",
    whyHack: "Voice without visual feedback feels like shouting into void - synchronized visual cues make voice interfaces feel responsive and trustworthy.",
    score: 28
  },
  {
    id: 267,
    title: "Haptic Feedback Mapping",
    description: "Design tactile feedback patterns for mobile interactions",
    category: "Design",
    example: "Map haptic patterns for [APP]: success (sharp tap), error (double buzz), selection (light tick), confirmation (heavy thud), progress (rhythmic pulses). Test pattern distinctiveness.",
    whyHack: "Haptics add a dimension most apps ignore - distinctive tactile patterns create memorable interactions that reinforce visual/audio feedback.",
    score: 27
  },
  {
    id: 268,
    title: "Dark Mode Contrast Rules",
    description: "Establish principles for maintaining hierarchy in dark interfaces",
    category: "Design",
    example: "Define dark mode contrast rules for [DESIGN_SYSTEM]: background layers (max 3 levels), text contrast (minimum per level), accent saturation adjustment, elevation shadows. Audit existing components.",
    whyHack: "Dark mode isn't inverted light mode - proper dark mode requires rethinking contrast relationships that break when simply flipping colors.",
    score: 26
  },
  {
    id: 269,
    title: "Responsive Breakpoint Philosophy",
    description: "Design breakpoints around content, not arbitrary device widths",
    category: "Design",
    example: "Analyze [LAYOUT] to find natural breakpoints: where does content break first? Define breakpoints by content needs, not device categories. Document the 'why' for each breakpoint.",
    whyHack: "Device-based breakpoints chase moving targets - content-based breakpoints adapt naturally to any screen because they're defined by what users actually see.",
    score: 25
  },
  {
    id: 270,
    title: "Scroll-Triggered Animation Timing",
    description: "Orchestrate animations that respond to scroll position",
    category: "Design",
    example: "Design scroll-triggered animations for [PAGE]: entry threshold (when visible), progress mapping (how scroll controls animation), exit behavior, mobile performance considerations.",
    whyHack: "Scroll animations create narrative momentum through content - properly timed scroll triggers make pages feel responsive to user exploration.",
    score: 24
  },
  {
    id: 271,
    title: "Brand Personality in UI Motion",
    description: "Express brand attributes through animation characteristics",
    category: "Design",
    example: "Translate [BRAND_PERSONALITY] into motion: playful (bounce, overshoot), professional (ease-in-out, restrained), innovative (spring, unexpected paths). Define duration ranges and curve families.",
    whyHack: "Motion is the voice of your UI - consistent animation personality reinforces brand identity at every interaction without explicit branding.",
    score: 23
  },
  {
    id: 272,
    title: "Accessibility-First Component Design",
    description: "Build components where accessibility is foundational, not retrofitted",
    category: "Design",
    example: "Design [COMPONENT] accessibility-first: start with screen reader flow, add keyboard navigation, layer visual design. Document: focus states, ARIA labels, announcement timing.",
    whyHack: "Retrofitting accessibility creates bolted-on experiences - accessibility-first design often produces better UX for everyone, not just those who need it.",
    score: 22
  },
  {
    id: 273,
    title: "Onboarding Flow Optimization",
    description: "Design onboarding that maximizes activation while minimizing abandonment",
    category: "Design",
    example: "Optimize [PRODUCT] onboarding: identify critical activation metric, map shortest path to 'aha moment', design skip points, measure each step's abandonment rate. A/B test order.",
    whyHack: "Every onboarding step is a potential exit - ruthless optimization removes friction to first value while preserving necessary education.",
    score: 21
  },
  {
    id: 274,
    title: "Feedback Loop Visualization",
    description: "Make system state changes visible and comprehensible",
    category: "Design",
    example: "Visualize feedback loops in [SYSTEM]: input → processing → result → impact on next input. Design visual language that makes causality chains obvious to users.",
    whyHack: "Hidden feedback loops create user confusion about cause-and-effect - visible loops build mental models that make complex systems feel learnable.",
    score: 20
  },
  {
    id: 275,
    title: "Neubrutalism Balance Techniques",
    description: "Apply raw neubrutalism aesthetics while maintaining usability",
    category: "Design",
    example: "Apply neubrutalism to [INTERFACE] while maintaining usability: bold colors (with accessibility), hard shadows (with depth logic), raw typography (with hierarchy). Balance edge with function.",
    whyHack: "Neubrutalism's visual impact often sacrifices usability - careful balance creates distinctive interfaces that are still functional and accessible.",
    score: 19
  }
];

export const categories = Array.from(new Set(hackPrompts.map(prompt => prompt.category))).sort();
